...
static int compute_maximum_coins(const std :: vector <std :: vector <int>> &, const int, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        std :: cout << compute_maximum_coins(matrix, nrows, ncols) << NEW_LINE;
    }
    return 0;
}
...
static int compute_maximum_coins(const std :: vector <std :: vector <int>> & matrix, const int nrows, const int ncols) {
    ...
    for(int i = 1; i < nrows; ++i) {
        for(int j = 0; j < ncols; ++j) {
            if(!j) {
                cost_matrix.at(j) += matrix[i][j];
                continue;
            }
            cost_matrix.at(j) = std :: max(matrix[i][j] + cost_matrix.at(j - 1), matrix[i][j] + cost_matrix.at(j));
        }
    }
