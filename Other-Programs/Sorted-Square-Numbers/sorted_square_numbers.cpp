...
static void generate_sorted_square_sequence(std :: vector <int> &, const size_t);
...
static void generate_sorted_square_sequence(std :: vector <int> & sequence, const size_t n) {
    std :: vector <int> negative_sequence, positive_sequence;
    for(int & number: sequence) {
        if(number < 0) {
            negative_sequence.push_back(number);
            continue;
        }
        positive_sequence.push_back(number);
    }
    if(negative_sequence.size() && positive_sequence.size()) {
        for(size_t i = negative_sequence.size() - 1, j = 0, k = 0; k < n; ++k) {
            if(i < 0) {
                sequence.at(k) = (positive_sequence[j] * positive_sequence[j]);
                ++j;
                continue;
            }
            if(j == positive_sequence.size()) {
                sequence.at(k) = (negative_sequence[i] * negative_sequence[i]);
                --i;
                continue;
            }
