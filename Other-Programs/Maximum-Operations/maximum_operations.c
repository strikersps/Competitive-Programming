...
static int compute_min(const int a, const int b) {
    return a < b ? a : b;
}
...
int main(void) {
    ...
    while(test--) {
        ...
        qsort(numbers, MAX_LIMIT, sizeof(int), comparator_callback);
        printf("%d\n", compute_min(numbers[0] + numbers[1], (numbers[0] + numbers[1] + numbers[2]) >> 1));
    }
    return EXIT_SUCCESS;
}
