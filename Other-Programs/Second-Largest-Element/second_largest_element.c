...
static int find_second_largest(int[], const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%d\n", find_second_largest(sequence, n));
    }
    return EXIT_SUCCESS;
}

static int find_second_largest(int sequence[], const int n) {
    int max_val, second_max_val;
    max_val = second_max_val = INT_MIN;
    for(register int i = 0; i < n; ++i) {
        if(max_val < sequence[i]) {
            second_max_val = max_val;
            max_val = sequence[i];
        } else if(max_val != sequence[i] && second_max_val < sequence[i]) {
            second_max_val = sequence[i];
        }
    }
