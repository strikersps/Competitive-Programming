...
static void generate_fibonacci_sequence(std :: vector <ll_t> &, const int);

int main(void) {
    FAST_IO(0);
    int n;
    std :: cin >> n;
    std :: vector <ll_t> fibonacci_sequence(n);
    generate_fibonacci_sequence(fibonacci_sequence, n);
    ll_t s_n = 3;
    for(int i = 1; i < n; ++i) {
        s_n += (fibonacci_sequence[i] * fibonacci_sequence[i]);
    }
    std :: cout << s_n << NEW_LINE;
    return 0;
}

static void generate_fibonacci_sequence(std :: vector <ll_t> & fibonacci_sequence, const int n) {
