...
static void compute_smallest_prime_factors(std :: vector <int> &);

int main(void) {
    FAST_IO(0);
    std :: vector <int> smallest_prime_factors(MAX_LIMIT, 0);
    compute_smallest_prime_factors(smallest_prime_factors);
    int test;
    std :: cin >> test;
    while(test--) {
        int number;
        std :: cin >> number;
        std :: map <int, int> prime_factorisation;
        for(int number_copy = number; smallest_prime_factors[number_copy]; ++prime_factorisation[smallest_prime_factors[number_copy]], number_copy /= smallest_prime_factors[number_copy]);
        for(std :: pair <const int, int> & mapping: prime_factorisation) {
...
static void compute_smallest_prime_factors(std :: vector <int> & smallest_prime_factors) {
    std :: vector <bool> prime_numbers(MAX_LIMIT, true);
    prime_numbers[0] = prime_numbers[1] = false;
    for(ll_t i = 2LL; i < MAX_LIMIT; ++i) {
        if(prime_numbers[i]) {
            smallest_prime_factors[i] = (int) i;
            for(ll_t j = i * i; j < MAX_LIMIT; j += i) {
                prime_numbers[j] = false;
                smallest_prime_factors[j] = (int) i;
            }
        }
    }
}
