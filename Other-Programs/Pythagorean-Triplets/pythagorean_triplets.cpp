/*
 * Problem Statement: Check if there exist a triplets (a, b, c) such that a^2 + b^2 = c^2.
 * Author: striker
 *
...
static void check_pythagorean_triplets(const std :: vector <int> &, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        check_pythagorean_triplets(sequence, n);
    }
    return 0;
}

static void check_pythagorean_triplets(const std :: vector <int> & sequence, const int n) {
    bool is_exist = false;
    std :: vector <int> triplets(3);
    for(int c_index = n - 1; c_index > 1; --c_index) {
        for(int start = 0, end = c_index - 1; start < end; ) {
            ll_t a = sequence[start] * sequence[start], b = sequence[end] * sequence[end], c = sequence[c_index] * sequence[c_index];
            // std :: cout << "a: " << a << " b: " << b << " c: " << c << NEW_LINE;
            if((a + b) == c) {
                is_exist = true;
                triplets[0] = sequence[start];
                triplets[1] = sequence[end];
                triplets[2] = sequence[c_index];
                break;
            }
            if((a + b) > c) {
                --end;
    ...
    if(!is_exist) {
        std :: cout << "Pythagorean Triplets doesn't Exist." << NEW_LINE;
    } else {
        std :: cout << triplets[0] << " " << triplets[1] << " " << triplets[2] << NEW_LINE;
    }
}
