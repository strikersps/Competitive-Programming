...
static void segregate_even_odd(std :: vector <int> &, const int);
...
static void segregate_even_odd(std :: vector <int> & sequence, const int n) {
    std :: queue <int> odd_numbers;
    int even_index = 0;
    for(int index = 0; index < n; ++index) {
        if(!(sequence[index] % 2)) {
            sequence[even_index++] = sequence[index];
        } else {
            odd_numbers.push(sequence[index]);
        }
    }
    while(even_index < n) {
        sequence[even_index++] = odd_numbers.front();
        odd_numbers.pop();
    }
}
