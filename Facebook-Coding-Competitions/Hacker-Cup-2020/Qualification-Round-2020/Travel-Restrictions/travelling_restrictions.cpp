...
static void compute_trip_matrix(const std :: vector <char> &, const std :: vector <char> &, std :: vector <std :: vector <char>> &, const int);
...
int main(void) {
    ...
    for(int t = 1; t <= test; ++t) {
        int n;
        std :: cin >> n;
        std :: vector <char> incoming(n), outgoing(n);
        for(auto & allowed: incoming) {
            std :: cin >> allowed;
        }
        for(auto & allowed: outgoing) {
            std :: cin >> allowed;
        }
        std :: vector <std :: vector <char>> trip_matrix(n, std :: vector <char> (n, 'N'));
        compute_trip_matrix(incoming, outgoing, trip_matrix, n);
        std :: cout << "Case #" << t << ": " << NEW_LINE;
        for(std :: vector <char> & row: trip_matrix) {
            for(auto & allowed: row) {
                std :: cout << allowed;
            }
            std :: cout << NEW_LINE;
        }
...
static void compute_trip_matrix(const std :: vector <char> & incoming, const std :: vector <char> & outgoing, std :: vector <std :: vector <char>> & trip_matrix, const int n) {
    for(int i = 0; i < n; ++i) {
        trip_matrix[i][i] = 'Y';
    }
    for(int i = 0; i < n; ++i) {
        if(outgoing.at(i) == 'Y') {
            if(i > 0) {
                for(int j = i - 1; j >= 0 && incoming.at(j) == 'Y'; --j) {
                    trip_matrix[i][j] = 'Y';
                    if(outgoing.at(j) != 'Y') {
                        break;
                    }
            ...
            if(i < n - 1) {
                for(int j = i + 1; j < n && incoming.at(j) == 'Y'; ++j) {
                    trip_matrix[i][j] = 'Y';
                    if(outgoing.at(j) != 'Y') {
                        break;
                    }
