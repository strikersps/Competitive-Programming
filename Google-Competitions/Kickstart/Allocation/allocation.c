...
int main(void) {
    ...
    for(register int t = 1; t <= test; ++t) {
        int total_houses, total_budget;
        if(2 != scanf("%d%d", &total_houses, &total_budget)) {
            SCANF_READ_ERROR(2);
        }
        int house_cost[total_houses], tot_money = 0;
        for(register int i = 0; i < total_houses; ++i) {
        ...
        if(tot_money <= total_budget) {
            printf("Case #%d: %d\n", t, total_houses);
            continue;
        }
        qsort(house_cost, (size_t) total_houses, sizeof(int), comparator_callback);
        int price_sum = 0, max_houses;
        for(register int i = 0; i < total_houses; ++i) {
            price_sum += house_cost[i];
            if(price_sum > total_budget) {
                max_houses = i;
                break;
            }
        }
