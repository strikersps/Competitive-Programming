...
static bool check_three_digit_divisors(const int);
static void generate_six_digit_palindromic_number(std :: vector <int> &);
...
int main(void) {
    ...
    std :: cin >> test;
    assert(test > 0);
    std :: vector <int> palindrome_numbers;
    generate_six_digit_palindromic_number(palindrome_numbers);
    while(test--) {
        int n;
        std :: cin >> n;
        assert(n > 101101 && n < 1000000);
        std :: cout << *(--std :: lower_bound(palindrome_numbers.begin(), palindrome_numbers.end(), n)) << NEW_LINE;
    }
    return 0;
}

static bool check_three_digit_divisors(const int n) {
    bool is_present = false;
    for(int d = 1; d <= n / d; ++d) {
        if(!(n % d) && d != (n / d)) {
            int d1 = d, d2 = n / d;
            if((d1 > 99 && d1 < 1000) && (d2 > 99 && d2 < 1000)) {
                is_present = true;
                break;
            }
        }
...
static void generate_six_digit_palindromic_number(std :: vector <int> & palindrome_numbers) {
    for(int a = 1; a < 10; ++a) {
        for(int b = 0; b < 10; ++b) {
            for(int c = 0; c < 10; ++c) {
                int palindromic_number = 11 * (9091 * a + 910 * b + 100 * c);
                if(check_three_digit_divisors(palindromic_number)) {
                    palindrome_numbers.push_back(palindromic_number);
                }
            }
        }
    }
