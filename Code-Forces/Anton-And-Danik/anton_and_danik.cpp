...
int main(void) {
    ...
    std :: cin >> game_result;
    int count_A = std :: count(game_result.begin(), game_result.end(), 'A');
    int count_B = std :: count(game_result.begin(), game_result.end(), 'D');
    std :: cout << (count_A == count_B ? "Friendship" : count_A > count_B ? "Anton" : "Danik") << std :: endl;
    return 0;
}
