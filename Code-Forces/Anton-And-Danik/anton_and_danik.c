...
int main(void) {
    ...
    char *const game_result = calloc((size_t) n + 1, sizeof*game_result);
    if(!game_result) {
        MEMORY_ALLOCATION_FAILED_ERROR(game_result, (size_t) (n + 1) * sizeof*game_result);
        exit(0);
    }
    if(1 != scanf("%s", game_result)) {
        SCANF_READ_ERROR(1);
    }
    int count_A, count_B;
    count_A = count_B = 0;
    ...
    printf("%s\n", (count_A == count_B ? "Friendship" : (count_A > count_B) ? "Anton" : "Danik"));
    free(game_result);
    return EXIT_SUCCESS;
}
