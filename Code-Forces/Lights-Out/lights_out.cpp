...
static void compute_final_state_light(const std :: vector <std :: vector <int>> &);
...
int main(void) {
    ...
    compute_final_state_light(matrix);
    return 0;
}

static void compute_final_state_light(const std :: vector <std :: vector <int>> & matrix) {
    for(int i = 1; i < 4; ++i) {
        for(int j = 1; j < 4; ++j) {
            std :: cout << (1 + matrix[i][j] + matrix[i - 1][j] + matrix[i + 1][j] + matrix[i][j - 1] + matrix[i][j + 1]) % 2;
        }
        std :: cout << NEW_LINE;
    }
}
