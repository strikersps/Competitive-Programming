/*
 * Problem Statement: https://codeforces.com/contest/682/problem/A
 * Author: striker
 *
...
static ll_t compute_number_of_pairs(const int, const int);

int main(void) {
    FAST_IO(0);
    int n, m;
    std :: cin >> n >> m;
    std :: cout << compute_number_of_pairs(n, m) << NEW_LINE;
    return 0;
}

static ll_t compute_number_of_pairs(const int n, const int m) {
    ll_t pair_count = 0LL;
    for(int i = 1; i <= n; ++i) {
