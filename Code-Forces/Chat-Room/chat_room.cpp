...
int main(void) {
    ...
    std :: string msg;
    std :: cin >> msg;
    const std :: string check_string("hello");
    bool is_check_string_present = false;
    int match_count = 0, i = 0;
    for(auto & letter: msg) {
        if(letter == check_string[i]) {
            ++i;
            ++match_count;
        }
        if(match_count == (int) check_string.size()) {
            is_check_string_present = true;
            break;
        }
    }
    std :: cout << (is_check_string_present ? "YES" : "NO") << NEW_LINE;
    return 0;
}
