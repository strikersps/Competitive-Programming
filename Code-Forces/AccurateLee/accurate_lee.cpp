/*
 * Problem Statement: https://codeforces.com/contest/1369/problem/B
 * Author: striker
 *
...
static int compute_leading_zeroes(const std :: string &, const int);
static int compute_trailing_ones(const std :: string &, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        for(int i = 0; i < n - 1; ++i) {
            if(binary_string.at(i) == '1' && binary_string.at(i + 1) == '0') {
                pair_exist = true;
                break;
            }
        }
        if(pair_exist) {
            int count_leading_zeroes = compute_leading_zeroes(binary_string, n), count_trailing_ones = compute_trailing_ones(binary_string, n);
            for(int i = 0; i <= count_leading_zeroes; ++i) {
                std :: cout << '0';
            }
            for(int i = 0; i < count_trailing_ones; ++i) {
                std :: cout << '1';
            }
            std :: cout << NEW_LINE;
            continue;
        }
        std :: cout << binary_string << NEW_LINE;
    }
    return 0;
}

static int compute_leading_zeroes(const std :: string & binary_string, const int n) {
    if(binary_string[0] == '1') {
        return 0;
    }
    int count_leading_zeros = 1;
    for(int i = 1; i < n; ++i) {
...
static int compute_trailing_ones(const std :: string & binary_string, const int n) {
    if(binary_string[n - 1] == '0') {
        return 0;
    }
    int count_trailing_ones = 1;
    for(int i = n - 2; i >= 0; --i) {
