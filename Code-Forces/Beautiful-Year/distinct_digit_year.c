...
#define MAX_LIMIT 10
#define YEAR_MAX_LIMIT 9100

static bool distinct_digit_check(int);
static int compute_distinct_year(const int);
...
int main(void) {
    ...
    printf("%d\n", compute_distinct_year(year));
    return EXIT_SUCCESS;
}

static bool distinct_digit_check(int n) {
    bool has_distinct_digit = true;
    bool frequency_map[MAX_LIMIT];
    INITIALISE_INT_CONTAINER_ZERO(frequency_map, sizeof(bool) * MAX_LIMIT);
    while(n) {
        if(frequency_map[n % 10]) {
            has_distinct_digit = false;
            break;
        }
        frequency_map[n % 10] = true;
        n /= 10;
    }
    return has_distinct_digit;
}

static int compute_distinct_year(const int year) {
    int distinct_year = 1234;
    for(int y = year + 1; y < YEAR_MAX_LIMIT; ++y) {
        if(distinct_digit_check(y)) {
            distinct_year = y;
            break;
        }
    }
