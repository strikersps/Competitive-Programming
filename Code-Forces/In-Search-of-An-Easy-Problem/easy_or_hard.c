...
int main(void) {
    ...
    int *const opinion = calloc((size_t) n, sizeof*opinion);
    if(!opinion) {
        MEMORY_ALLOCATION_FAILED_ERROR(opinion, (size_t) n * sizeof*opinion);
        exit(0);
    }
    bool is_easy = true;
    for(int i = 0; i < n; ++i) {
        if(1 != scanf("%d", opinion + i)) {
            SCANF_READ_ERROR(1);
        }
        if(is_easy && opinion[i]) {
            is_easy = false;
        }
    }
    printf("%s\n", is_easy ? "EASY" : "HARD");
