...
int main(void) {
    ...
    bool frequency_letter[NUMBER_OF_ALPHABETS];
    INITIALISE_INT_CONTAINER_ZERO(frequency_letter, NUMBER_OF_ALPHABETS * sizeof(bool));
    for(int i = 0; user_name[i] != '\0'; ++i) {
        if(!frequency_letter[user_name[i] - 'a']) {
            frequency_letter[user_name[i] - 'a'] = 1;
        }
    }
    int unique_letter_count = 0;
    for(int i = 0; i < NUMBER_OF_ALPHABETS; ++i) {
        if(frequency_letter[i]) {
            ++unique_letter_count;
        }
    }
    printf("%s\n", (unique_letter_count % 2 ? "IGNORE HIM!" : "CHAT WITH HER!"));
    return EXIT_SUCCESS;
}
