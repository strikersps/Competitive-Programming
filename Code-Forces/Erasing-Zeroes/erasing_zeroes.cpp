/*
 * Problem Statement: https://codeforces.com/problemset/problem/1303/A
 * Author: striker
 *
...
static int compute_min_zeroes_removed(const std :: string &);
...
int main(void) {
    ...
    while(test--) {
        ...
        std :: cout << compute_min_zeroes_removed(binary_string) << std :: endl;
    }
    return 0;
}

static int compute_min_zeroes_removed(const std :: string & binary_string) {
    if(binary_string.size() == 1) {
        return 0;
    }
    int start = 0, end = binary_string.size() - 1;
    while(start <= end) {
        if(binary_string.at(start) == '0') {
            ++start;
            continue;
        }
        if(binary_string.at(end) == '0') {
            --end;
        }
        if(binary_string.at(start) == '1' && binary_string.at(end) == '1') {
            break;
        }
    }
    int zero_count = 0;
