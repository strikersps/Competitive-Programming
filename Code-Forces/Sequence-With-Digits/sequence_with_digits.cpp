/*
 * Problem Statement: https://codeforces.com/contest/1355/problem/A
 * Author: striker
 *
...
static bool check_zero_digit(ll_t);
static int compute_max_digit(ll_t);
static int compute_min_digit(ll_t);
static ll_t compute_nth_term(const ll_t, const ll_t);
...
int main(void) {
    ...
    while(test--) {
        ll_t n, k;
        std :: cin >> n >> k;
        std :: cout << compute_nth_term(n, k) << NEW_LINE;
    }
    return 0;
}

static bool check_zero_digit(ll_t n) {
    ...
    return has_zero_digit;
}

static int compute_max_digit(ll_t n) {
    int max_digit = 0;
    while(n) {
        int digit = n % 10LL;
        if(digit > max_digit) {
...
static int compute_min_digit(ll_t n) {
    int min_digit = 9;
    while(n) {
        int digit = n % 10LL;
        if(digit < min_digit) {
...
static ll_t compute_nth_term(ll_t term, const ll_t k) {
    std :: vector <ll_t> sequence;
    for(ll_t i = 0; true; ++i) {
        if(check_zero_digit(term)) {
            sequence.push_back(term);
            break;
        }
        sequence.push_back(term);
        term = term + (compute_max_digit(term) * compute_min_digit(term));
    }
    if((ll_t) sequence.size() < k) {
        return sequence.at(sequence.size() - 1);
    }
    return sequence.at(k - 1);
}
