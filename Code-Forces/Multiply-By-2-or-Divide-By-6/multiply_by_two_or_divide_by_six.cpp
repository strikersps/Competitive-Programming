/*
 * Problem Statement: https://codeforces.com/contest/1374/problem/B
 * Author: striker
 *
...
static int compute_minimum_moves(int);
...
int main(void) {
    ...
    while(test--) {
        int n;
        std :: cin >> n;
        std :: cout << compute_minimum_moves(n) << NEW_LINE;
    }
    return 0;
}

static int compute_minimum_moves(int n) {
    int n_copy = n, count_twos = 0, count_threes = 0;
    while(!(n_copy % 2)) {
        ++count_twos;
        n_copy /= 2;
    }
    ...
    if(n_copy > 1 || (count_twos > count_threes)) {
        return -1;
    }
    int min_value = std :: min(count_twos, count_threes);
    return min_value + 2 * (std :: max(count_twos, count_threes) - min_value);
}
