...
int main(void) {
    ...
    ll_t sum_of_faces = 0;
    while(n--) {
        char polyhedron_name[MAX_STRING_LENGTH];
        if(1 != scanf("%s", polyhedron_name)) {
            SCANF_READ_ERROR(1);
        }
        if(!strncmp(polyhedron_name, "Tetrahedron", MAX_STRING_LENGTH)) {
            sum_of_faces += 4;
            continue;
        }
        if(!strncmp(polyhedron_name, "Cube", MAX_STRING_LENGTH)) {
            sum_of_faces += 6;
            continue;
        }
        if(!strncmp(polyhedron_name, "Octahedron", MAX_STRING_LENGTH)) {
            sum_of_faces += 8;
            continue;
        }
        if(!strncmp(polyhedron_name, "Dodecahedron", MAX_STRING_LENGTH)) {
            sum_of_faces += 12;
            continue;
        }
        sum_of_faces += 20;
