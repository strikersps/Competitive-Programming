...
int main(void) {
    ...
    int binary_string_len = (int) strlen(binary_string);
    int consecutive_zeros = 1, consecutive_ones = 1;
    for(int i = 0; i < binary_string_len - 1; ++i) {
        if(binary_string[i] == '0' && binary_string[i + 1] == '0') {
            ++consecutive_zeros;
            continue;
        }
        if(binary_string[i] == '1' && binary_string[i + 1] == '1') {
            ++consecutive_ones;
            continue;
        }
        if(consecutive_zeros > 6 || consecutive_ones > 6) {
            break;
        }
        consecutive_zeros = consecutive_ones = 1;
    }
    printf("%s\n", (consecutive_zeros > 6 || consecutive_ones > 6 ? "YES" : "NO"));
    return EXIT_SUCCESS;
}
