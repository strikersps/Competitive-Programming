/*
 * Problem Statement: https://codeforces.com/contest/1341/problem/A
 * Author: striker
 *
...
int main(void) {
    ...
    while(test--) {
        int n, a, b, c, d;
        std :: cin >> n >> a >> b >> c >> d;
        int min_wt_pack = n * abs(a - b);
        int max_wt_pack = n * (a + b);
        if((min_wt_pack >= abs(c - d) && min_wt_pack <= (c + d)) || (max_wt_pack >= abs(c - d) && max_wt_pack <= (c + d)) || (abs(c - d) >= min_wt_pack && abs(c - d) <= max_wt_pack) || ((c + d) >= min_wt_pack && (c + d) <= max_wt_pack)) {
