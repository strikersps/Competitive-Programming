...
static std :: string borze_code_decoder(const std :: string &);

int main(void) {
    FAST_IO(0);
    std :: string borze_code;
    std :: cin >> borze_code;
    std :: cout << borze_code_decoder(borze_code) << NEW_LINE;
    return 0;
}

static std :: string borze_code_decoder(const std :: string & borze_code) {
    int borze_code_len = borze_code.size();
    std :: string decoded_string;
    for(int index = 0; index < borze_code_len; ++index) {
        if(borze_code.at(index) == '.') {
            decoded_string += '0';
            continue;
        }
        if(index < (borze_code_len - 1) && borze_code[index++] == '-') {
            if(borze_code.at(index) == '.') {
                decoded_string += '1';
            } else {
                decoded_string += '2';
