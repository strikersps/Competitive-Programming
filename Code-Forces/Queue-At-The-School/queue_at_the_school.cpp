...
static void swap_data(char &, char &);
static void transform_queue(std :: string, const int);

int main(void) {
    FAST_IO(0);
    int n, t;
    std :: cin >> n >> t;
    std :: string initial_queue;
    std :: cin >> initial_queue;
    transform_queue(initial_queue, t);
    return 0;
}

static void swap_data(char & a, char & b) {
    char temp;
    temp = a;
    a = b;
    b = temp;
...
static void transform_queue(std :: string updated_queue, const int t) {
    for(int i = 0; i < t; ++i) {
        for(int index = 1; index < (int) updated_queue.size(); ++index) {
            if(updated_queue.at(index) == 'G' && updated_queue.at(index - 1) == 'B') {
                swap_data(updated_queue.at(index), updated_queue.at(index - 1));
                ++index;
            }
        }
    }
    std :: cout << updated_queue << NEW_LINE;
}
