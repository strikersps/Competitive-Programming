/*
 * Problem Statement: https://codeforces.com/gym/287428/problem/C
 * Author: striker
 *
...
static void transform_queue(char *const, const int, const int);
...
int main(void) {
    ...
    assert(n > 0 && t > 0);
    char *const queue_state = calloc((size_t) n + 1, sizeof*queue_state);
    if(!queue_state) {
        MEMORY_ALLOCATION_FAILED_ERROR(queue_state, (size_t) n * sizeof*queue_state);
        exit(0);
    }
    if(1 != scanf("%s", queue_state)) {
        SCANF_READ_ERROR(1);
    }
    transform_queue(queue_state, n, t);
    puts(queue_state);
    free(queue_state);
    return EXIT_SUCCESS;
}

static void swap_characters(char *const a, char *const b) {
    char temp = *a;
    *a = *b;
    *b = temp;
}

static void transform_queue(char *const initial_queue_state, const int n, const int t) {
    for(int i = 0; i < t; ++i) {
        for(int j = 1; j < n; ++j) {
            if(initial_queue_state[j] == 'G' && initial_queue_state[j - 1] == 'B') {
                swap_characters(&initial_queue_state[j - 1], &initial_queue_state[j]);
                ++j;
            }
        }
    }
