...
static void convert_to_lowercase(char *const string_data) {
    size_t string_len = strlen(string_data);
    for(int i = 0; i < (int) string_len; ++i) {
        if(isupper(string_data[i])) {
            string_data[i] = (char) tolower(string_data[i]);
        }
    }
}
