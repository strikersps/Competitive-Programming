...
int main(void) {
    ...
    std :: transform(string1.begin(), string1.end(), string1.begin(), [](char c) {return std :: tolower(c);});
    std :: transform(string2.begin(), string2.end(), string2.begin(), [] (char c) {return std :: tolower(c);});
    int ret_value = string1.compare(string2);
    std :: cout << (ret_value < 0 ? -1 : ret_value > 0 ? 1 : 0) << NEW_LINE;
    return 0;
}
