/*
 * Problem Statement: https://codeforces.com/contest/1374/problem/A
 * Author: striker
 *
...
static int compute_maximum_k(int, int, int);
...
int main(void) {
    ...
    while(test--) {
        int x, y, n;
        std :: cin >> x >> y >> n;
        std :: cout << compute_maximum_k(x, y, n) << NEW_LINE;
    }
    return 0;
}

static int compute_maximum_k(int x, int y, int n) {
    int remainder = n % x;
    if(remainder == y) {
        return n;
    }
