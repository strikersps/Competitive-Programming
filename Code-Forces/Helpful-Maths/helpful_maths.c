/*
 * Problem Statement: https://codeforces.com/contest/339/problem/A
 * Author: striker
 *
...
static void rearrange_expression(const char*);
...
static void rearrange_expression(const char *expression) {
    int ones_count, twos_count, threes_count;
    ones_count = twos_count = threes_count = 0;
    ...
    int expression_len = ones_count + twos_count + threes_count;
    for(int i = 0; i < expression_len; ++i) {
        if(ones_count) {
            ...
            printf("2");
            --twos_count;
        } else if(threes_count) {
            printf("3");
            --threes_count;
        }
        if(i < (expression_len - 1)) {
            printf("+");
        }
    }
    printf("\n");
