/*
 * Problem Statement: https://codeforces.com/contest/1399/problem/A
 * Author: striker
 *
...
static bool check_possibility(const std :: vector <int> &, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        std :: cout << (check_possibility(sequence, n) ? "YES" : "NO") << NEW_LINE;
    }
    return 0;
}

static bool check_possibility(const std :: vector <int> & sequence, const int n) {
    if(n == 1) {
        return true;
    }
