/*
 * Problem Statement: https://codeforces.com/contest/1353/problem/B
 * Author: striker
 *
...
static int compute_maximum_sum(std :: vector <int>, std :: vector <int>, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        std :: cout << compute_maximum_sum(s1, s2, k) << NEW_LINE;
    }
    return 0;
}
...
static int compute_maximum_sum(std :: vector <int> s1, std :: vector <int> s2, const int k) {
    for(int i = 0; i < k; ++i) {
        if(s1[i] < s2[i]) {
            swap_data(s1[i], s2[i]);
        }
