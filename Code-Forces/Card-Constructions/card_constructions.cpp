/*
 * Problem Statement: https://codeforces.com/problemset/problem/1345/B
 * Author: striker
 *
...
#define MAX_VECTOR_SIZE 25820 // Computed by solving the quadratic equation 3*h^2 + h - 2*10^9 = 0. Refer: https://www.wolframalpha.com/input/?i=solve+3x%5E2+%2B+x+-+2000000000

static ll_t compute_total_pyramid_constructed(const std :: vector <ll_t> &, int);

int main(void) {
    FAST_IO(0);
    std :: vector <ll_t> total_cards(MAX_VECTOR_SIZE); // Store total cards needed to build a pyramid of height i.
    for(int i = 0; i < MAX_VECTOR_SIZE; ++i) {
        total_cards.at(i) = ((3 * (i + 1) * (i + 1)) + i + 1LL) / 2LL;
    }
    int test;
    std :: cin >> test;
    while(test--) {
        ...
        std :: cout << compute_total_pyramid_constructed(total_cards, number_of_cards) << NEW_LINE;
    }
    return 0;
}

static ll_t compute_total_pyramid_constructed(const std :: vector <ll_t> & total_cards, int number_of_cards) {
    ll_t total_pyramid_count = 0LL;
    for(int limit = std :: upper_bound(total_cards.begin(), total_cards.end(), number_of_cards) - total_cards.begin() - 1; ~limit; --limit) {
        total_pyramid_count += number_of_cards / total_cards[limit];
        number_of_cards %= total_cards[limit];
    }
    return total_pyramid_count;
}
