/*
 * Problem Statement: https://codeforces.com/problemset/problem/630/J
 * Author: striker
 *
...
static ll_t compute_sequence_lcm(const std :: vector <int> &);

int main(void) {
    FAST_IO(0);
    std :: vector <int> sequence = {2, 3, 4, 5, 6, 7, 8, 9, 10};
    ll_t n;
    std :: cin >> n;
    std :: cout << n / compute_sequence_lcm(sequence) << NEW_LINE;
    return 0;
}
...
static ll_t compute_sequence_lcm(const std :: vector <int> & sequence) {
    ll_t sequence_lcm = 1LL;
    for(const int & number: sequence) {
        sequence_lcm = (sequence_lcm * number) / compute_gcd(number, sequence_lcm);
    }
    return sequence_lcm;
}
