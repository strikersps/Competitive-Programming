...
static void generate_required_sequence(const std :: vector <int> &, const std :: vector <int> &, const std :: vector <int> &);
...
int main(void) {
    ...
    while(test--) {
        int n;
        std :: cin >> n;
        std :: vector <int> sequence1(n), sequence2(n), sequence3(n);
        for(int & number: sequence1) {
            std :: cin >> number;
        }
        for(int & number: sequence2) {
            std :: cin >> number;
        }
        for(int & number: sequence3) {
            std :: cin >> number;
        }
        generate_required_sequence(sequence1, sequence2, sequence3);
    }
    return 0;
}

static void generate_required_sequence(const std :: vector <int> & sequence1, const std :: vector <int> & sequence2, const std :: vector <int> & sequence3) {
    int n = (int) sequence1.size();
    std :: vector <int> resultant_sequence(n);
    for(int i = 0, j = 0, k = 0, l = 0; l < n; ++i, ++j, ++k, ++l) {
        if(!l) {
            resultant_sequence[l] = sequence1[i];
            continue;
        }
        if(sequence1[i] != resultant_sequence[0] && sequence1[i] != resultant_sequence[l - 1]) {
            resultant_sequence[l] = sequence1[i];
            continue;
        }
        if(sequence2[j] != resultant_sequence[0] && sequence2[j] != resultant_sequence[l - 1]) {
            resultant_sequence[l] = sequence2[j];
            continue;
        }
        resultant_sequence[l] = sequence3[k];
    }
    for(int i = 0; i < n; ++i) {
        if(!i) {
            std :: cout << resultant_sequence[i];
            continue;
        }
        std :: cout << " " << resultant_sequence[i];
    }
