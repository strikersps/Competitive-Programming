...
static int compute_color_conflicts(int);

int main(void) {
    FAST_IO(0);
    int n;
    std :: cin >> n;
    std :: cout << compute_color_conflicts(n) << NEW_LINE;
    return 0;
}

static int compute_color_conflicts(int n) {
    std :: unordered_map <int, int> freq_map_home, freq_map_away;
    while(n--) {
        int home_color, away_color;
        std :: cin >> home_color >> away_color;
        ++freq_map_home[home_color];
        ++freq_map_away[away_color];
    }
    int conflict_count = 0;
    for(const std :: pair <int, int> & mapping: freq_map_home) {
        conflict_count += (mapping.second * freq_map_away[mapping.first]);
    }
