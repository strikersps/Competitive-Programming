...
int main(void) {
    FAST_IO(0);
    int total_dollars;
    std :: cin >> total_dollars;
    int count_denominations = 0;
    while(total_dollars) {
        if(total_dollars >= 100) {
            count_denominations += (total_dollars / 100);
            total_dollars %= 100;
            continue;
        }
        if(total_dollars >= 20) {
            count_denominations += (total_dollars / 20);
            total_dollars %= 20;
            continue;
        }
        if(total_dollars >= 10) {
            count_denominations += (total_dollars / 10);
            total_dollars %= 10;
            continue;
        }
        if(total_dollars >= 5) {
            count_denominations += (total_dollars / 5);
            total_dollars %= 5;
            continue;
        }
        count_denominations += total_dollars;
        total_dollars = 0;
    }
    std :: cout << count_denominations << std :: endl;
    return 0;
}
