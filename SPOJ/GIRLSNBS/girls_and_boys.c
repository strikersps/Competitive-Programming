...
static int find_min(const int, const int);
static int find_max(const int, const int);

int main(void) {
    while(true) {
        int tot_boys, tot_girls;
        if(2 != scanf("%d%d", &tot_boys, &tot_girls)) {
            SCANF_READ_ERROR(2);
        }
        if(tot_boys == -1 && tot_girls == -1) {
            break;
        }
        if(!tot_girls || !tot_boys) {
            printf("%d\n", find_max(tot_boys, tot_girls));
            continue;
        }
        if(tot_girls == tot_boys) {
            printf("%d\n", 1);
        ...
        printf("%d\n", (tot_boys + tot_girls) / (find_min(tot_boys, tot_girls) + 1));
    }
    return EXIT_SUCCESS;
}
...
static int find_max(const int a, const int b) {
    return a > b ? a : b;
}
