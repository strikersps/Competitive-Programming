...
static ll_t compute_trailing_zeroes(ll_t);
...
int main(void) {
    ...
    while(test--) {
        ll_t n;
        std :: cin >> n;
        std :: cout << compute_trailing_zeroes(n) << NEW_LINE;
    }
    return 0;
}
...
static ll_t compute_trailing_zeroes(ll_t n) {
    ...
    ll_t count_trailing_zeroes = 0LL, power_of_five = compute_power(5LL, 1LL);
    for(ll_t i = 2LL; n / power_of_five; ++i) {
        count_trailing_zeroes += (n / power_of_five);
        power_of_five = compute_power(5LL, i);
    }
