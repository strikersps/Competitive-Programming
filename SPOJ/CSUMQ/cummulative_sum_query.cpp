...
static void execute_queuries(const std :: vector <int> &, const int);
...
static void execute_queuries(const std :: vector <int> & sequence, const int n) {
    std :: vector <ll_t> prefix_sum(n, 0);
    prefix_sum.at(0) = sequence.at(0);
    for(int i = 1; i < n; ++i) {
        prefix_sum.at(i) = prefix_sum.at(i - 1) + sequence.at(i);
    }
    int queries;
    std :: cin >> queries;
    while(queries--) {
