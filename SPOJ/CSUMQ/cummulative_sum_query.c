...
static void execute_queuries(const int *, const int);
...
static void execute_queuries(const int *sequence, const int n) {
    ll_t *const prefix_sum = calloc((size_t) n, sizeof*prefix_sum);
    if(!prefix_sum) {
        MEMORY_ALLOCATION_FAILED_ERROR(prefix_sum, (size_t) n * sizeof*prefix_sum);
        exit(0);
    }
    ...
    int queries;
    if(1 != scanf("%d", &queries)) {
        SCANF_READ_ERROR(1);
    }
    while(queries--) {
        int start_range, end_range;
        if(2 != scanf("%d%d", &start_range, &end_range)) {
            SCANF_READ_ERROR(2);
        }
        assert(start_range <= end_range);
        printf("%lld\n", !start_range ? prefix_sum[end_range] : prefix_sum[end_range] - prefix_sum[start_range - 1]);
    }
}
