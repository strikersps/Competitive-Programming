...
static bool check_behive_number(const int);
...
int main(void) {
    ...
    while(true) {
        ...
        printf("%s\n", n == 1 ? "Y" : check_behive_number(n) ? "Y" : "N");
    }
    return EXIT_SUCCESS;
}

static bool check_behive_number(const int n) {
    if((n - 1) % 3) {
        return false;
    }
    bool is_behive = false;
    int quotient = (n - 1) / 3;
    for(register int i = 1; i * i <= quotient; ++i) {
        if(!(quotient % i)) {
            int d1 = i;
            int d2 = quotient / i;
            if((d2 - d1) == 1) {
