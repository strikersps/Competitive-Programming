...
int main(void) {
    ...
    while(test--) {
        int n;
        std :: cin >> n;
        std :: vector <std :: string> accnts_data(n);
        for(std :: string & accnt_number: accnts_data) {
            std :: getline(std :: cin, accnt_number);
            while(!accnt_number.size()) {
                std :: getline(std :: cin, accnt_number);
            }
        }
        std :: sort(accnts_data.begin(), accnts_data.end());
        std :: map <std :: string, int> data;
        for(const std :: string & accnts: accnts_data) {
            data[accnts] += 1;
        }
        for(std :: pair <const std :: string, int> & mapping: data) {
            std :: cout << mapping.first << " " << mapping.second << NEW_LINE;
        }
        if(test) {
            std :: cout << NEW_LINE;
        }
