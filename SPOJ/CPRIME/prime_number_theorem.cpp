/*
 * Problem Statement: https://www.spoj.com/problems/CPRIME/
 * Author: striker
 *
...
int main(void) {
    ...
    while(true) {
        ...
        int pi_of_x = std :: upper_bound(prime_numbers.begin(), prime_numbers.end(), x) - prime_numbers.begin();
        printf("%0.1f\n", (fabs(pi_of_x - (x / logl(x))) / pi_of_x) * 100);
    }
    return 0;
}

static void prime_generator(std :: vector <int> & prime_numbers) {
    std :: vector <bool> sieve_list(MAX_LIMIT, true);
