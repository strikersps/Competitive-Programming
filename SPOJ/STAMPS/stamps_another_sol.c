...
int main(void) {
    ...
    if(1 == scanf("%d", &scenarios)) {
        for(register int scenario = 1; scenario <= scenarios; ++scenario) {
            int stamps_required, tot_friends;
            if(2 == scanf("%d%d", &stamps_required, &tot_friends)) {
                int stamp_offered[tot_friends], tot_stamps_offered = 0;
                for(register int i = 0; i < tot_friends; ++i) {
                ...
                qsort(stamp_offered, (size_t) tot_friends, sizeof(int), comparator_callback);
                int min_stamp_required = 0;
