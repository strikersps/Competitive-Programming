...
static ll_t compute_number_of_ways(const std :: string &);
...
int main(void) {
    ...
    while(true) {
        ...
        std :: cout << compute_number_of_ways(encoded_msg) << NEW_LINE;
    }
    return 0;
}

static ll_t compute_number_of_ways(const std :: string & encoded_msg) {
    int encoded_msg_len = encoded_msg.size();
    if(encoded_msg_len == 1) {
        return 1;
    }
    bool is_possible = true;
    std :: vector <ll_t> number_of_ways(encoded_msg_len);
    number_of_ways.at(0) = 1;
    for(int i = 1; i < encoded_msg_len; ++i) {
        const short int code = 10 * (encoded_msg.at(i - 1) - '0') + encoded_msg.at(i) - '0';
        if(encoded_msg.at(i) == '0' && (!code || code > 26)) {
            is_possible = false;
            break;
        }
        if(encoded_msg.at(i) == '0') {
            number_of_ways.at(i) = ((i > 1) ? number_of_ways.at(i - 2) : 1LL);
            continue;
        }
        if(code > 26 || encoded_msg.at(i - 1) == '0') {
            number_of_ways.at(i) = number_of_ways.at(i - 1);
            continue;
        }
        number_of_ways.at(i) = number_of_ways.at(i - 1) + (i > 1 ? number_of_ways.at(i - 2) : 1LL);
    }
    if(!is_possible) {
        return 0;
    }
    return number_of_ways.at(encoded_msg_len - 1);
}
