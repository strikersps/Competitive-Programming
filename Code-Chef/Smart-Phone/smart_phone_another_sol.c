...
static ll_t compute_maximum_profit(ll_t[], const int);
...
int main(void) {
    ...
    for(register int i = 0; i < n; ++i) {
        if(1 != scanf("%lld", budget + i)) {
            SCANF_READ_ERROR(1);
        }
    }
    qsort(budget, (size_t) n, sizeof(ll_t), comparator_callback);
    printf("%lld\n", compute_maximum_profit(budget, n));
    return EXIT_SUCCESS;
}

static ll_t compute_maximum_profit(ll_t budget[], const int n) {
    ll_t max_profit = 0;
    for(register int i = 0; i < n; ++i) {
        ll_t revenue_earned = budget[i] * (n - i);
        max_profit = FIND_MAX(max_profit, revenue_earned);
    }
    return max_profit;
}
