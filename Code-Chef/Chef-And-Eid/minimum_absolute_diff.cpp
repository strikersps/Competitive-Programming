...
static int compute_minimum_absolute_difference(const std :: vector <int> &, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        std :: cout << compute_minimum_absolute_difference(sequence, n) << std :: endl;
    }
    return 0;
}

static int compute_minimum_absolute_difference(const std :: vector <int> & sequence, const int n) {
    int min_diff = std :: numeric_limits<int> :: max();
    for(int i = n - 1; i > 0; --i) {
        min_diff = std :: min(min_diff, sequence[i] - sequence[i - 1]);
    }
    return min_diff;
}
