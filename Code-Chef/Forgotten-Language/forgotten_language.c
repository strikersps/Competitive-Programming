...
int main(void) {
    ...
    if(1 == scanf("%d", &test)) {
        while(test--) {
            int n, k;
            if(2 == scanf("%d%d", &n, &k)) {
                phrase_t frgt_phrase[n];
                for(register int i = 0; i < n; ++i) {
                    if(1 == scanf("%s", frgt_phrase[i].word)) {
                        frgt_phrase[i].is_present = false;
                        continue;
                    }
                    SCANF_READ_ERROR(1);
                }
                for(register int i = 0; i < k; ++i) {
                    int phrase_length;
                    if(1 != scanf("%d", &phrase_length)) {
                        SCANF_READ_ERROR(1);
                    }
                    char read_word[MAX_WORD_LENGTH];
                    for(register int j = 0; j < phrase_length; ++j) {
                        if(1 != scanf("%s", read_word)) {
                            SCANF_READ_ERROR(1);
                        }
                        for(register int l = 0; l < n; ++l) {
                            if(!strcmp(frgt_phrase[l].word, read_word)) {
                                frgt_phrase[l].is_present = true;
                                break;
                            }
                        }
                    }
                ...
                for(register int i = 0; i < n; ++i) {
                    printf("%s ", frgt_phrase[i].is_present ? "YES" : "NO");
                }
                printf("\n");
            } else {
                SCANF_READ_ERROR(2);
