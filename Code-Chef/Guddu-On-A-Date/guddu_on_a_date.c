...
static ull_t compute_sum_of_digit(ull_t);
...
int main(void) {
    ...
    while(test--) {
        ...
        ull_t sum_of_digit = compute_sum_of_digit(n);
        printf("%llu\n", (10 * n) + (sum_of_digit % 10 ? 10 - (sum_of_digit % 10) : 0));
    }
    return EXIT_SUCCESS;
}

static ull_t compute_sum_of_digit(ull_t n) {
    ull_t sum_of_digit = 0;
    while(n) {
        sum_of_digit += n % 10;
        n /= 10;
    }
