...
static void compute_max_array(int[], const int, int *const, int *const);
        ...
        while(test--) {
            ...
            if(1 == scanf("%d", &n)) {
                ...
                for(register int i = 0; i < n; ++i) {
                    if(1 != scanf("%d", salary + i)) {
                        SCANF_READ_ERROR(1);
                    }
                }
                int min_val, max_val, max_val_index;
                min_val = INT_MAX; max_val = -1;
                for(register int i = 0; i < n; ++i) {
                    if(salary[i] < min_val) {
                        min_val = salary[i];
                        continue;
                ...
                compute_max_array(salary, n, &max_val, &max_val_index);
                int delta = max_val - min_val;
                int min_operations = 0;
                while(delta) {
                    min_operations += delta;
                    for(register int i = 0; i < n; ++i) {
                        if(i == max_val_index) {
                            continue;
                        }
...
static void compute_max_array(int data[], const int n, int *const max_val, int *const max_val_index) {
    for(register int i = 0; i < n; ++i) {
        if(data[i] > (*max_val)) {
            *max_val = data[i];
            *max_val_index = i;
