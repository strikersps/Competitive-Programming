...
static int compute_card_power(int);
static std :: pair <int, int> determine_game_winner(int);
...
int main(void) {
    ...
    while(test--) {
        int n;
        std :: cin >> n;
        std :: pair <int, int> winner_details = determine_game_winner(n);
        std :: cout << winner_details.first << " " << winner_details.second << NEW_LINE;
    }
    return 0;
}

static int compute_card_power(int n) {
    int card_power = 0;
    while(n) {
        card_power += (n % 10);
        n /= 10;
...
static std :: pair <int, int> determine_game_winner(int n) {
    int chef_points = 0, morty_points = 0;
    while(n--) {
        int card1, card2;
        std :: cin >> card1 >> card2;
        const int card1_power = compute_card_power(card1);
        const int card2_power = compute_card_power(card2);
        if(card1_power > card2_power) {
            ++chef_points;
        } else if (card1_power < card2_power) {
            ++morty_points;
        } else {
            ++chef_points, ++morty_points;
        }
    }
    return (chef_points > morty_points) ? std :: make_pair(0, chef_points) : (chef_points < morty_points) ? std :: make_pair(1, morty_points) : std :: make_pair(2, chef_points);
}
