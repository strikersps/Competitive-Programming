/*
 * Problem Statement: https://www.codechef.com/AUG20B/problems/CHEFWED
 * Author: striker
 *
...
static int compute_minimum_inefficiency(const std :: vector <int> &, const int, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        std :: cout << compute_minimum_inefficiency(sequence, n, k) << NEW_LINE;
    }
    return 0;
}

static int compute_minimum_inefficiency(const std :: vector <int> & sequence, const int n, const int k) {
    std :: map <int, int> repetition;
    std :: vector <std :: vector <int>> argument(n, std :: vector <int> (n, 0));
    for(int i = 0; i < n; ++i) {
        repetition.clear();
        for(int j = i; j < n; ++j) {
            argument[i][j] = !j ? 0 : argument[i][j - 1];
            if(repetition.count(sequence.at(j))) {
                if(repetition[sequence.at(j)] == 1) {
                    ++argument[i][j];
                }
                ++argument[i][j];
            }
            ++repetition[sequence.at(j)];
        }
    }
    const int table = 100;
    std :: vector <std :: vector <int>> dp(101, std :: vector <int> (n + 1, 0));
    for(int i = 1; i < n + 1; ++i) {
        dp[1][i] = argument[0][i - 1];
    }
    for(int i = 2; i <= table; ++i) {
        for(int j = 2; j <= n; ++j) {
            dp[i][j] = std :: numeric_limits <int> :: max();
            for(int k = 1; k < j; ++k) {
                dp[i][j] = std :: min(dp[i][j], dp[i - 1][k] + argument[k][j - 1]);
            }
        }
    }
    int min_inefficiency = std :: numeric_limits <int> :: max();
    for(int t = 1; t <= table; ++t) {
        min_inefficiency = std :: min(min_inefficiency, t * k + dp[t][n]);
    }
    return min_inefficiency;
}
