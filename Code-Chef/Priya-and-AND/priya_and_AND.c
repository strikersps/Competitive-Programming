/*
 * Problem Statement: https://www.codechef.com/CENS2020/problems/CENS20D
 * Author: striker
 *
...
static int compute_pairs(const int *, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%d\n", compute_pairs(sequence, n));
        free(sequence);
    }
    return EXIT_SUCCESS;
}

static int compute_pairs(const int *sequence, const int n) {
    int count_pairs = 0;
    for(int i = 0; i < n - 1; ++i) {
        for(int j = i + 1; j < n; ++j) {
            if(sequence[i] == (sequence[i] & sequence[j])) {
                ++count_pairs;
            }
        }
    }
