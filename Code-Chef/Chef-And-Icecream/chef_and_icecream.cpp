...
static bool check_serve_all_possible(const std :: vector <int> &, const int);
...
int main(void) {
    ...
    while(test--) {
        int n;
        std :: cin >> n;
        std :: vector <int> person_budget(n);
        for(int & budget: person_budget) {
            std :: cin >> budget;
        }
        std :: cout << (check_serve_all_possible(person_budget, n) ? "YES" : "NO") << NEW_LINE;
    }
    return 0;
}

static bool check_serve_all_possible(const std :: vector <int> & person_budget, const int n) {
    bool is_possible = true;
    std :: vector <int> change_count(3, 0);
    for(const int & budget: person_budget) {
        if(budget == 5) {
            ++change_count[0];
            continue;
        }
