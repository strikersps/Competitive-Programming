/*
 * Problem Statement: https://www.codechef.com/COMT2020/problems/EQUALNUM
 * Author: striker
 *
...
int main(void) {
    ...
    std :: cin >> test;
    while(test--) {
        std :: string given_number_international, given_number_indian, actual_number_international, actual_number_indian;
        std :: cin >> given_number_international >> given_number_indian;
        for(const auto & number: given_number_international) {
            if(number == ',') {
                continue;
            }
            actual_number_international += number;
        }
        for(const auto & number: given_number_indian) {
            if(number == ',') {
                continue;
            }
            actual_number_indian += number;
        }
        std :: cout << (actual_number_international == actual_number_indian ? "equal" : "different") << NEW_LINE;
    }
    return 0;
}
