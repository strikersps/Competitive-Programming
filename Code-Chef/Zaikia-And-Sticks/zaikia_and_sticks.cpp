/*
 * Problem Statement: https://www.codechef.com/COJK2020/problems/CKOJ20A
 * Author: striker
 *
...
int main(void) {
    ...
    int n;
    std :: cin >> n;
    std :: vector <int> stick_length(n);
    for(int & length: stick_length) {
        std :: cin >> length;
    }
    std :: sort(stick_length.rbegin(), stick_length.rend());
    bool is_triangle_possible = false;
    for(int i = 0; i <= n - 3; ++i) {
        if(stick_length[i + 2] + stick_length[i + 1] > stick_length[i]) {
            is_triangle_possible = true;
            std :: cout << "YES" << NEW_LINE;
            std :: cout << stick_length[i] << " " << stick_length[i + 1] << " " << stick_length[i + 2] << NEW_LINE;
            break;
        }
    }
    if(!is_triangle_possible) {
