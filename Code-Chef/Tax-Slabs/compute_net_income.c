...
const int tax_slab[][3] = { {250000, 0, 0},
                            {500000, 5, 12500},
                            {750000, 10, 25000},
                            {1000000, 15, 37500},
                            {1250000, 20, 50000},
...
static int compute_net_income(const int);
...
int main(void) {
    ...
    if(1 == scanf("%d", &test)) {
        ...
        while(test--) {
            int tot_income;
            if(1 == scanf("%d", &tot_income)) {
                if(tot_income < 0) {
                    CONSTRAINTS_OUT_OF_BOUND_ERROR(tot_income, tot_income cannot be -ve);
                    exit(0);
                }
                printf("%d\n", compute_net_income(tot_income));
            } else {
                SCANF_READ_ERROR(1);
            }
        }
...
static int compute_net_income(const int tot_income) {
    double tax_amt = 0.0;
    register int i;
    for(i = 0; i < 6; ++i) {
        if(tax_slab[i][0] <= tot_income) {
            tax_amt += (double) (tax_slab[i][2]);
            continue;
        }
        break;
    }
    if(i == 6) {
        tax_amt += ((double) (tot_income - tax_slab[i - 1][0]) * (0.30));
    } else {
        tax_amt += ((double) (tot_income - tax_slab[i - 1][0]) * ((double) tax_slab[i][1] / 100));
    }
    return tot_income - tax_amt;
}
