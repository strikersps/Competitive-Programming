/*
 * Problem Statement: https://www.codechef.com/JUNE20B/problems/EOEO
 * Author: striker
 *
...
static ll_t compute_number_of_ways(ll_t);
...
int main(void) {
    ...
    while(test--) {
        ll_t tom_strength;
        std :: cin >> tom_strength;
        std :: cout << compute_number_of_ways(tom_strength) << NEW_LINE;
    }
    return 0;
}

static ll_t compute_number_of_ways(ll_t tom_strength) {
    if(compute_mod(tom_strength, 2LL)) {
        return tom_strength / 2LL;
    }
    if(tom_strength && !(tom_strength & (tom_strength - 1LL))) {
        return 0;
    }
    ll_t copy_tom_strength = tom_strength, count_twos = 0LL;
    while(!compute_mod(copy_tom_strength, 2LL)) {
        ++count_twos;
        copy_tom_strength /= 2LL;
    }
    return tom_strength / (1LL << (count_twos + 1LL));
}
