...
int main(void) {
    ...
    if(2 == scanf("%d%d", &n, &m)) {
        ...
        int min_val_s1, min_index_s1;
        min_val_s1 = INF; min_index_s1 = -1;
        for(int i = 0; i < n; ++i) {
            if(1 == scanf("%d", sequence1 + i)) {
                if(sequence1[i] < min_val_s1) {
                    min_val_s1 = sequence1[i];
                    min_index_s1 = i;
                }
                continue;
            }
        ...
        int max_val_s2, max_index_s2;
        max_val_s2 = -INF; max_index_s2 = -1;
        for(int i = 0; i < m; ++i) {
            if(1 == scanf("%d", sequence2 + i)) {
                if(sequence2[i] > max_val_s2) {
                    max_val_s2 = sequence2[i];
                    max_index_s2 = i;
                }
                continue;
            }
