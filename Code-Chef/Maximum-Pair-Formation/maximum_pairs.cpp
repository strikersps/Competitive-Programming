...
int main(void) {
    ...
    std :: cin >> test;
    while(test--) {
        std :: string gender_students;
        std :: cin >> gender_students;
        int max_pairs = 0;
        for(int i = 0; i < (int) gender_students.size() - 1; ++i) {
            if(gender_students[i] != gender_students[i + 1]) {
                ++max_pairs;
                ++i;
                continue;
            }
        }
        std :: cout << max_pairs << NEW_LINE;
    }
    return 0;
}
