...
static int compute_total_cars(int[], const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        for(register int i = 0; i < n; ++i) {
            if(1 == scanf("%d", car_speed + i)) {
                continue;
            }
            SCANF_READ_ERROR(1);
        }
        printf("%d\n", compute_total_cars(car_speed, n));
    }
    return EXIT_SUCCESS;
}

static int compute_total_cars(int car_speed[], const int n) {
    int car_count = 1;
