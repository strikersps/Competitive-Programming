/*
 * Problem Statement: https://www.codechef.com/IIITDS20/problems/MAY19F1
 * Author: striker
 *
...
static int compute_maximum_score_after_n_games(int[], const int);
    ...
    while(test--) {
        ...
        if(2 == scanf("%d%d", &n, &q)) {
            ...
            for(register int i = 0; i < n; ++i) {
                if(1 != scanf("%d", score + i)) {
                    SCANF_READ_ERROR(1);
                }
            }
            for(register int i = 0; i < q; ++i) {
                int thought;
                if(1 != scanf("%d", &thought)) {
                    SCANF_READ_ERROR(1);
                }
                printf("%d\n", compute_maximum_score_after_n_games(score, thought));
            }
            continue;
        }
        SCANF_READ_ERROR(1);
...
static int compute_maximum_score_after_n_games(int score[], const int n) {
    int max_val = 0;
    for(register int i = 0; i < n; ++i) {
        if(score[i] > max_val) {
            max_val = score[i];
