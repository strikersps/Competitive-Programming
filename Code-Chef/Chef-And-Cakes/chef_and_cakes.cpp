/*
 * Problem Statement: https://www.codechef.com/QFUN2020/problems/CHEFCAKE
 * Author: striker
 *
...
static ll_t compute_factorial(const int);
static ll_t compute_power(ll_t, ll_t);
static ll_t compute_sum_of_permutations(const std :: vector <int> &, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        std :: cout << compute_sum_of_permutations(number, n) << NEW_LINE;
    }
    return 0;
}

static ll_t compute_factorial(const int n) {
    if(n == 1) {
        return 1LL;
    }
...
static ll_t compute_sum_of_permutations(const std :: vector <int> & number, const int n) {
    if(n == 1) {
        return number[0];
    }
    ll_t sum_of_digits_at_position = std :: accumulate(number.begin(), number.end(), 0LL) * compute_factorial(n) / n;
    ll_t sum_of_permutations = 0LL;
    for(int i = 0; i < n; ++i) {
        sum_of_permutations += (sum_of_digits_at_position * compute_power(10LL, i));
    }
