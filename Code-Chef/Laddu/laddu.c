...
int main(void) {
    ...
    while(test--) {
        ...
        if(2 != scanf("%d%s", &tot_activities, origin)) {
            SCANF_READ_ERROR(2);
        }
        int tot_laddus = 0;
        for(register int i = 0; i < tot_activities; ++i) {
            char activity_name[MAX_STRING_LENGTH];
            if(1 != scanf("%s", activity_name)) {
                SCANF_READ_ERROR(1);
            }
            if(!strcmp(activity_name, "CONTEST_WON")) {
                tot_laddus += 300;
                int your_rank;
                if(1 != scanf("%d", &your_rank)) {
                    SCANF_READ_ERROR(1);
                }
                if(your_rank <= 20) {
                    tot_laddus += (20 - your_rank);
                }
                continue;
            }
            if(!strcmp(activity_name, "TOP_CONTRIBUTOR")) {
                tot_laddus += 300;
                continue;
            }
            if(!strcmp(activity_name, "BUG_FOUND")) {
                int severity_val;
                if(1 != scanf("%d", &severity_val)) {
                    SCANF_READ_ERROR(1);
                }
                tot_laddus += severity_val;
                continue;
            }
            if(!strcmp(activity_name, "CONTEST_HOSTED")) {
                tot_laddus += 50;
                continue;
            }
        }
        printf("%d\n", !strcmp(origin, "INDIAN") ? tot_laddus / 200 : tot_laddus / 400);
    }
    return EXIT_SUCCESS;
}
