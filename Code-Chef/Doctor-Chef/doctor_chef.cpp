/*
 * Problem Statement: https://www.codechef.com/JULY20B/problems/DRCHEF
 * Author: striker
 *
...
static int compute_minimum_days(std :: vector <int> &, const int, int);
...
int main(void) {
    ...
    while(test--) {
        int n, x;
        std :: cin >> n >> x;
        std :: vector <int> population_count(n);
        for(int & pop_count: population_count) {
            std :: cin >> pop_count;
        }
        std :: sort(population_count.begin(), population_count.end());
        std :: cout << compute_minimum_days(population_count, n, x) << NEW_LINE;
    }
    return 0;
}

static int compute_minimum_days(std :: vector <int> & population_count, const int n, int cure_count) {
    int min_days = 0;
    while(true) {
        if(population_count.size() == 0) {
            break;
        }
        ++min_days;
        if(cure_count < population_count.at(0)) {
            cure_count *= 2;
            continue;
        }
        auto found_itr = std :: lower_bound(population_count.begin(), population_count.end(), cure_count);
        if(found_itr != population_count.end() && *found_itr == cure_count) {
            cure_count *= 2;
            population_count.erase(found_itr);
            continue;
        }
        --found_itr;
        int curr_cure_count = 2 * (*found_itr);
        if(found_itr + 1 != population_count.end() && curr_cure_count < cure_count) {
            cure_count *= 2;
            continue;
        }
        cure_count = curr_cure_count;
        population_count.erase(found_itr);
    }
    return min_days;
}
