/*
 * Problem Statement: https://www.codechef.com/LTIME86B/problems/CHECHOC
 * Author: striker
 *
...
static void generate_matrix(std :: vector <std :: vector <ll_t>> &, const ll_t, const ll_t, const ll_t, const ll_t);
...
int main(void) {
    ...
    while(test--) {
        ...
        std :: vector <std :: vector <ll_t>> matrix(n, std :: vector <ll_t> (m));
        generate_matrix(matrix, n, m, x, y);
        ll_t maximum_coins = 0LL;
        for(const std :: vector <ll_t> & row: matrix) {
            maximum_coins += std :: accumulate(row.begin(), row.end(), 0LL);
        }
        std :: cout << maximum_coins << NEW_LINE;
    }
    return 0;
}

static void generate_matrix(std :: vector <std :: vector <ll_t>> & matrix, const ll_t n, const ll_t m, const ll_t x, const ll_t y) {
    for(int i = 0; i < n; ++i) {
        if(!(i % 2)) {
            for(int j = 0; j < m; ++j) {
                if(!(j % 2)) {
