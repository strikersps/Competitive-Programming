/*
 * Problem Statement: https://www.codechef.com/AUG20B/problems/SKMP
 * Author: striker
 *
...
static void find_lexicographically_smallest_anagram(const std :: string &, const std :: string &);
...
int main(void) {
    ...
    std :: cin >> test;
    while(test--) {
        std :: string given_string, pattern_string;
        std :: cin >> given_string >> pattern_string;
        find_lexicographically_smallest_anagram(given_string, pattern_string);
    }
    return 0;
}

static void find_lexicographically_smallest_anagram(const std :: string & given_string, const std :: string & pattern_string) {
    std :: map <char, int> freq_map;
    for(const auto & character: given_string) {
        ++freq_map[character];
    }
    for(const auto & character: pattern_string) {
        --freq_map[character];
    }
    std :: string prefix_string, suffix_string;
    for(std :: pair <const char, int> & mapping: freq_map) {
        if(mapping.first < pattern_string[0]) {
            for(int i = 0; i < mapping.second; prefix_string += mapping.first, ++i);
            mapping.second = 0;
            continue;
        }
        if(mapping.first == pattern_string[0]) {
            bool comes_before = false;
            for(int i = 0; i < (int) pattern_string.size() - 1; ++i) {
                if(pattern_string[i] != pattern_string[i + 1]) {
                    comes_before = pattern_string[i + 1] > mapping.first ? true : false;
                    break;
                }
            }
            for(int i = 0; comes_before && i < mapping.second; prefix_string += mapping.first, ++i);
            if(comes_before) {
                mapping.second = 0;
            }
        }
    ...
    for(std :: pair <const char, int> & mapping: freq_map) {
        for(int i = 0; i < mapping.second; ++i) {
            suffix_string += mapping.first;
        }
    }
    std :: cout << prefix_string + pattern_string + suffix_string << NEW_LINE;
}
