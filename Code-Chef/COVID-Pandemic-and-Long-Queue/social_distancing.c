...
static bool check_social_distancing(int[], const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        for(register int i = 0; i < n; ++i) {
            if(1 != scanf("%d", people_queue + i)) {
                SCANF_READ_ERROR(1);
            }
        }
        printf("%s\n", check_social_distancing(people_queue, n) ? "YES" : "NO");
    }
    return EXIT_SUCCESS;
}

static bool check_social_distancing(int people_queue[], const int n) {
    bool is_social_distancing_followed = true;
    int last_person_index = 0;
    for(register int i = 0; i < n; ++i) {
        if(!last_person_index && people_queue[i]) {
            last_person_index = i + 1;
            continue;
        }
        if(last_person_index && people_queue[i]) {
            if(((i + 1) - last_person_index) < 6) {
