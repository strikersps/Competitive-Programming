...
static std :: pair <int, int> compute_winner(const int, const int);
...
int main(void) {
    ...
    while(test--) {
        int p_c, p_r;
        std :: cin >> p_c >> p_r;
        std :: pair <int, int> winner_details = compute_winner(p_c, p_r);
        std :: cout << winner_details.first << " " << winner_details.second << NEW_LINE;
    }
    return 0;
}

static std :: pair <int, int> compute_winner(const int p_c, const int p_r) {
    int digit_p_c = ceil((double) p_c / 9), digit_p_r = ceil((double) p_r / 9);
    std :: pair <int, int> winner_details;
    if(digit_p_c == digit_p_r) {
        return {1, digit_p_c};
    }
    return (digit_p_c > digit_p_r) ? std :: make_pair(1, digit_p_r) : std :: make_pair(0, digit_p_c);
}
