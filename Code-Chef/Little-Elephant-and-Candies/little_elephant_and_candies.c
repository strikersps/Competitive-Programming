...
int main(void) {
    ...
    if(1 == scanf("%d", &test)) {
        while(test--) {
            int tot_elephants, tot_candies;
            if(2 == scanf("%d%d", &tot_elephants, &tot_candies)) {
                int elephant_candy_need[tot_elephants];
                int tot_candies_required = 0;
                for(register int i = 0; i < tot_elephants; ++i) {
                    if(1 == scanf("%d", elephant_candy_need + i)) {
                        tot_candies_required += elephant_candy_need[i];
                        continue;
                    }
                    SCANF_READ_ERROR(1);
                }
                printf("%s\n", tot_candies_required <= tot_candies ? "Yes" : "No");
            } else {
                SCANF_READ_ERROR(2);
            }
        }
