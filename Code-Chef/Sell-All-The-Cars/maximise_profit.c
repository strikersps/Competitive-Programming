...
static int compute_maximum_profit(int[], const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        for(register int i = 0; i < n; ++i) {
            if(1 != scanf("%d", car_price + i)) {
                SCANF_READ_ERROR(1);
            }
        }
        qsort(car_price, (size_t) n, sizeof(int), comparator_callback);
        printf("%d\n", compute_maximum_profit(car_price, n));
    }
    return EXIT_SUCCESS;
}
...
static int compute_maximum_profit(int car_price[], const int n) {
    int max_profit = 0;
    for(register int i = 0; i < n; ++i) {
        if((car_price[i] - i) > 0) {
            car_price[i] -= i;
