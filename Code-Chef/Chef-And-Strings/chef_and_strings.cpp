...
int main(void) {
    ...
    while(test--) {
        int n;
        std :: cin >> n;
        std :: vector <int> guitar_string_index(n);
        for(int & number: guitar_string_index) {
            std :: cin >> number;
        }
        ll_t total_strings_skip = 0LL;
        for(int i = 0; i < n - 1; ++i) {
            total_strings_skip += (abs(guitar_string_index.at(i + 1) - guitar_string_index.at(i)) - 1LL);
        }
        std :: cout << total_strings_skip << NEW_LINE;
    }
    return 0;
}
