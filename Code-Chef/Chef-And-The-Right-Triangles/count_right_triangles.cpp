...
static ll_t compute_eucledian_distance(const std :: pair <int, int> &, const std :: pair <int, int> &);
static bool check_right_triangle(const std :: vector <std :: pair <int, int>> &);
static int compute_total_right_triangles(const std :: vector <std :: vector <std :: pair <int, int>>> &, const int);

int main(void) {
    FAST_IO(0);
    int n;
    std :: cin >> n;
    std :: vector <std :: vector <std :: pair <int, int>>> coordinates(n, std :: vector <std :: pair<int, int>> (3));
    for(auto & triangle_coordinate: coordinates) {
        for(auto & point: triangle_coordinate) {
            std :: cin >> point.first >> point.second;
        }
    }
    std :: cout << compute_total_right_triangles(coordinates, n) << std :: endl;
    return 0;
}

static int compute_total_right_triangles(const std :: vector <std :: vector <std :: pair <int, int>>> & coordinates, const int n) {
    int count_right_triangle = 0;
    for(const auto & triangle_coordinate: coordinates) {
        if(check_right_triangle(triangle_coordinate)) {
            ++count_right_triangle;
        }
    }
    return count_right_triangle;
}

static ll_t compute_eucledian_distance(const std :: pair <int, int> & p1, const std :: pair <int, int> & p2) {
    ll_t x_coordinate_diff = (p2.first - p1.first);
    x_coordinate_diff *= x_coordinate_diff;
    ll_t y_coordinate_diff = (p2.second - p1.second);
    y_coordinate_diff *= y_coordinate_diff;
    return x_coordinate_diff + y_coordinate_diff;
}

static bool check_right_triangle(const std :: vector <std :: pair <int, int>> & triangle_coordinate) {
    const int total_sides = 3;
    std :: vector <ll_t> triangle_side_length(total_sides);
    triangle_side_length.at(0) = compute_eucledian_distance(triangle_coordinate.at(0), triangle_coordinate.at(1));
    triangle_side_length.at(1) = compute_eucledian_distance(triangle_coordinate.at(1), triangle_coordinate.at(2));
    triangle_side_length.at(2) = compute_eucledian_distance(triangle_coordinate.at(2), triangle_coordinate.at(0));
    std :: sort(triangle_side_length.begin(), triangle_side_length.end());
    if(!triangle_side_length.at(0) || !triangle_side_length.at(1) || !triangle_side_length.at(2)) {
        return false;
    }
    return (triangle_side_length.at(0) + triangle_side_length.at(1) == triangle_side_length.at(2));
}
