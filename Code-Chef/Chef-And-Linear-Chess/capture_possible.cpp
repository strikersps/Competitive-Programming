...
int main(void) {
    ...
    while(test--) {
        int n, k;
        std :: cin >> n >> k;
        int min_value = std :: numeric_limits <int> :: max(), min_moves = -1;
        for(int i = 0; i < n; ++i) {
            int pawn_position;
            std :: cin >> pawn_position;
            if((k >= pawn_position) && !((k - pawn_position) % pawn_position) && ((k - pawn_position) / pawn_position) < min_value) {
                min_value = (k - pawn_position) / pawn_position;
                min_moves = pawn_position;
            }
        }
        std :: cout << min_moves << NEW_LINE;
    }
    return 0;
}
