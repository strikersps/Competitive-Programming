/*
 * Problem Statement: https://www.codechef.com/COJK2020/problems/CKOJ20B
 * Author: striker
 *
...
static ll_t compute_power(ll_t, ll_t);
static ll_t compute_sum(const std :: vector <int> &, const int, const int, const int);
static int compute_minimum_x_value(const std :: vector <int> &, const int, const int);
...
int main(void) {
    ...
    std :: cout << compute_minimum_x_value(sequence, n, k) << NEW_LINE;
    return 0;
}

static ll_t compute_power(ll_t base, ll_t exponent) {
    if(!exponent) {
        return 1LL;
    }
    ll_t result = 1;
    for(; exponent; exponent >>= 1LL) {
        if(exponent % 2) {
            result *= base;
        }
        base *= base;
...
static ll_t compute_sum(const std :: vector <int> & sequence, const int n, const int x, const int k) {
    ll_t sum = 0;
    for(int i = 0; i < n; ++i) {
        sum += compute_power(abs(x - sequence.at(i)), k);
    }
    return sum;
}

static int compute_minimum_x_value(const std :: vector <int> & sequence, const int n, const int k) {
    int start = sequence.at(0), end = sequence.at(n - 1);
    int x_value = -1;
    while(start <= end) {
        int mid = start + (end - start) / 2;
        ll_t x1 = compute_sum(sequence, n, mid, k);
        ll_t x2 = compute_sum(sequence, n, mid - 1, k);
        ll_t x3 = compute_sum(sequence, n, mid + 1, k);
        if(x2 > x1 && x3 > x1) {
            x_value = mid;
            break;
        } else if(x1 < x2) {
