...
int main(void) {
    ...
    while(test--) {
        int n, q;
        std :: cin >> n >> q;
        std :: string virus_infected_people;
        std :: cin >> virus_infected_people;
        std :: unordered_map <char, int> frequency_map;
        for(const char & virus_name: virus_infected_people) {
            ++frequency_map[virus_name];
        }
        while(q--) {
            int c, queue_length;
            std :: cin >> c;
            ...
            for(char key = 'a'; key <= 'z'; ++key) {
                if(frequency_map[key] > c) {
                    queue_length += (frequency_map[key] - c);
                }
            }
