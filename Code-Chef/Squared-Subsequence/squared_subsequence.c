...
static ll_t compute_total_subsequence(const int[], const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%lld\n", compute_total_subsequence(sequence, n));
    }
    return EXIT_SUCCESS;
}

static ll_t compute_total_subsequence(const int sequence[], const int n) {
    ll_t subsequence_count = 0;
    for(int i = 0; i < n; ++i) {
        if(sequence[i] % 2 || !(sequence[i] % 4)) {
            ++subsequence_count;
        }
        ll_t product = sequence[i];
        for(int j = i + 1; j < n; ++j) {
            product *= sequence[j];
            if(product % 2 || !(product % 4)) {
                ++subsequence_count;
                continue;
            }
        }
