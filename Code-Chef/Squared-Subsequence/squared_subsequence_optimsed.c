...
static ll_t compute_total_number_of_subarrays(int[], const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%lld\n", compute_total_number_of_subarrays(sequence, n));
    }
    return EXIT_SUCCESS;
}

static ll_t compute_total_number_of_subarrays(int sequence[], const int n) {
    ll_t subarray_till[n], subarray_count = 0;
    INITIALISE_INT_CONTAINER_ZERO(subarray_till, (size_t) n * sizeof(int));
    for(int i = 0, last_even_loc = -1; i < n; ++i) {
        if(sequence[i] % 2) {
            if(!i) {
                subarray_till[i] = 1;
            } else {
                subarray_till[i] = subarray_till[i - 1] + 1;
            }
        } else {
            if(!(sequence[i] % 4)) {
                subarray_till[i] = i + 1;
            } else {
                subarray_till[i] = last_even_loc + 1;
            }
            last_even_loc = i;
        }
        subarray_count += subarray_till[i];
    }
