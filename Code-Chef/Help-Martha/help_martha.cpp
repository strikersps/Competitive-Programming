...
int main(void) {
    ...
    while(test--) {
        ...
        std :: unordered_map <char, int> direction_count;
        for(const auto & direction: direction_string) {
            ++direction_count[direction];
        }
        std :: pair <int, int> start_position;
        std :: cin >> start_position.first >> start_position.second;
        int total_queries;
        for(std :: cin >> total_queries; total_queries; --total_queries) {
            std :: pair <int, int> final_position;
            std :: cin >> final_position.first >> final_position.second;
            int x_coordinate_diff = final_position.first - start_position.first;
            int y_coordinate_diff = final_position.second - start_position.second;
            if((!x_coordinate_diff || (x_coordinate_diff > 0 && x_coordinate_diff <= direction_count['R']) || (x_coordinate_diff < 0 && abs(x_coordinate_diff) <= direction_count['L'])) && (!y_coordinate_diff || (y_coordinate_diff > 0 && y_coordinate_diff <= direction_count['U']) || (y_coordinate_diff < 0 && abs(y_coordinate_diff) <= direction_count['D']))) {
                std :: cout << "YES " << abs(x_coordinate_diff) + abs(y_coordinate_diff) << NEW_LINE;
            } else {
