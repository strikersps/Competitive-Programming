/*
 * Problem Statement: https://www.codechef.com/problems/GIFTSRC
 * Author: striker
 *
...
int main(void) {
    ...
    while(test--) {
        ...
        char instruction[n + 1];
        if(1 != scanf("%s", instruction)) {
            SCANF_READ_ERROR(1);
        }
        point_2d_t position = {0, 0};
        bool on_x_axis, on_y_axis;
        on_x_axis = on_y_axis = false;
        for(register int i = 0; instruction[i] != '\0'; ++i) {
