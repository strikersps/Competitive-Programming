...
static int count_number_of_heads(char[], const int, const int);
    ...
    if(1 == scanf("%d", &test)) {
        ...
        while(test--) {
            ...
            if(2 == scanf("%d%d", &n, &k)) {
                ...
                char read_char;
                int i = 0;
                while((read_char = getchar_unlocked()) != '\n') {
                    if(read_char == ' ') {
                        continue;
                    }
                    coin_state[i++] = read_char;
                }
                printf("%d\n", count_number_of_heads(coin_state, n, k));
            } else {
                SCANF_READ_ERROR(2);
            }
        }
...
static int count_number_of_heads(char coin_state[], const int n, const int k) {
    if(coin_state[n - k] == 'H') {
