...
static int compute_maximum_difference(const int *, const int, const int);
...
int main(void) {
    ...
    while(test--) {
        int n, k;
        if(2 == scanf("%d%d", &n, &k)) {
            int *const item_wt = calloc((size_t) n, sizeof*item_wt);
            if(!item_wt) {
                MEMORY_ALLOCATION_FAILED_ERROR(item_wt, (size_t) n * sizeof*item_wt);
                exit(0);
            }
            for(int i = 0; i < n; ++i) {
                if(1 != scanf("%d", item_wt + i)) {
                    SCANF_READ_ERROR(1);
                }
            }
            qsort(item_wt, (size_t) n, sizeof*item_wt, qsort_comparator_callback);
            printf("%d\n", compute_maximum_difference(item_wt, n, k));
            free(item_wt);
            continue;
        }
        SCANF_READ_ERROR(2);
...
static int compute_maximum_difference(const int *item_wt, const int n, const int k) {
    int max_items = FIND_MAX(k, n - k);
    int total_item_wt = 0;
    for(int i = 0; i < n; ++i) {
        total_item_wt += item_wt[i];
    }
    int wt_sum_chef = 0;
    for(int i = n - 1; i >= (n - max_items); --i) {
        wt_sum_chef += item_wt[i];
    }
    return wt_sum_chef - (total_item_wt - wt_sum_chef);
}
