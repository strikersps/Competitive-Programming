...
static int compute_maximum_difference(const std :: vector <int> &, const int, const int);
...
int main(void) {
    ...
    while(test--) {
        int n, k;
        std :: cin >> n >> k;
        std :: vector <int> item_wt(n);
        for(int & weight: item_wt) {
            std :: cin >> weight;
        }
        std :: sort(item_wt.begin(), item_wt.end());
        std :: cout << compute_maximum_difference(item_wt, n, k) << NEW_LINE;
    }
    return 0;
}

static int compute_maximum_difference(const std :: vector <int> & item_wt, const int n, const int k) {
    int max_items = std :: max(k, n - k);
    int wt_sum_chef = std :: accumulate(item_wt.rbegin(), item_wt.rbegin() + max_items, 0);
    return wt_sum_chef - (std :: accumulate(item_wt.begin(), item_wt.end(), 0) - wt_sum_chef);
}
