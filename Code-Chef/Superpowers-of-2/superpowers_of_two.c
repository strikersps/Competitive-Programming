...
static ull_t compute_binary_equivalent(int);
static ll_t compute_modular_exponentiation(ll_t, ull_t);
...
int main(void) {
    ...
    while(test--) {
        ...
        ull_t nth_power = compute_binary_equivalent(n);
        ll_t base = compute_modular_exponentiation(2, nth_power);
        printf("%lld\n", (base * base) % MOD);
    }
    return EXIT_SUCCESS;
}

static ull_t compute_binary_equivalent(int n) {
    int number_of_bits = 2 + (int) log2(n);
    char binary_form[number_of_bits];
    INITIALISE_CHAR_CONTAINER_ZERO(binary_form, (unsigned long) number_of_bits * sizeof(char));
    for(register int i = number_of_bits - 2; i >= 0;) {
        binary_form[i--] = (char) ('0' + (n % 2));
        n >>= 1;
    }
    binary_form[number_of_bits - 1] = '\0';
    return strtoull(binary_form, NULL, 10);
}

static ll_t compute_modular_exponentiation(ll_t base, ull_t exponent) {
    ll_t result = 1;
    while(exponent) {
        if(exponent & 1) {
