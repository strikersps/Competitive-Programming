...
static bool check_multiple_of_three(const ll_t, const ll_t, const ll_t);
...
int main(void) {
    ...
    while(test--) {
        ll_t k, d0, d1;
        if(3 == scanf("%lld%lld%lld", &k, &d0, &d1)) {
            printf("%s\n", check_multiple_of_three(k, d0, d1) ? "YES" : "NO");
            continue;
        }
        SCANF_READ_ERROR(3);
    }
...
static bool check_multiple_of_three(const ll_t k, const ll_t d0, const ll_t d1) {
    if(k == 2 || !((d0 + d1) % 10)) {
        return !((d0 + d1) % 3);
    }
    ll_t sum_of_digit = d0 + d1 + ((d0 + d1) % 10);
    if(k > 3) {
        ll_t digit_repeat[] = {((d0 + d1) << 1) % 10, ((d0 + d1) << 2) % 10, ((d0 + d1) << 3) % 10, ((d0 + d1) << 4) % 10};
        sum_of_digit += (((k - 3) / 4) * (digit_repeat[0] + digit_repeat[1] + digit_repeat[2] + digit_repeat[3]));
        switch((k - 3) % 4) {
            case 1:
