...
static ll_t compute_xor(const ll_t); // Compute the xor of n consecutive numbers from [1, n].
...
int main(void) {
    ...
    if(1 == scanf("%d", &test)) {
        ...
        while(test--) {
            ll_t start_range, end_range;
            if(2 == scanf("%lld%lld", &start_range, &end_range)) {
                if(start_range > end_range) {
                    fprintf(stderr, "Line number: %u: Constraints not satisfied, i.e. start <= end.\n", __LINE__);
                    exit(0);
                }
                printf("%s\n", (compute_xor(end_range) ^ compute_xor(start_range - 1)) & 1 ? "Odd" : "Even");
            } else {
                SCANF_READ_ERROR(2);
            }
        }
...
static ll_t compute_xor(const ll_t n) {
    unsigned int last_two_digit = n % 100;
    unsigned xor_till_n;
    switch(last_two_digit % 4) {
        case 0:
            xor_till_n = n;
            break;
        case 1:
