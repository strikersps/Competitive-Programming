...
static int compute_maximum_coins(const int, const int);
...
int main(void) {
    ...
    while(test--) {
        int n, k;
        std :: cin >> n >> k;
        std :: cout << compute_maximum_coins(n, k) << NEW_LINE;
    }
    return 0;
}

static int compute_maximum_coins(const int n, const int k) {
    int max_coins = 0;
    for(int i = 1; i <= k; ++i) {
