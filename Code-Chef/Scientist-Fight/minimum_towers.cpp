/*
 * Problem Statement: https://www.codechef.com/QFUN2020/problems/SCTFIGHT
 * Author: striker
 *
...
int main(void) {
    ...
    while(test--) {
        int n;
        std :: cin >> n;
        std :: vector <int> cube_length(n);
        std :: map <int, int> freq_map;
        for(int & side_length: cube_length) {
            std :: cin >> side_length;
            ++freq_map[side_length];
        }
        int max_element = std :: numeric_limits <int> :: min();
        for(const std :: pair <int, int> mapping: freq_map) {
            if(mapping.second > max_element) {
                max_element = mapping.second;
            }
        }
        std :: cout << max_element << NEW_LINE;
    }
    return 0;
}
