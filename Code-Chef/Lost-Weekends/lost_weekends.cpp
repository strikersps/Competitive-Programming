/*
 * Problem Statement: https://www.codechef.com/LTIME84B/problems/LOSTWKND
 * Author: striker
 *
...
int main(void) {
    ...
    while(test--) {
        std :: vector <int> work_hrs(weekdays);
        for(int & number: work_hrs) {
            std :: cin >> number;
        }
        int p, hrs_spent_house = 0;
        std :: cin >> p;
        for(const int & number: work_hrs) {
            hrs_spent_house += (number * p);
        }
        std :: cout << (hrs_spent_house <= (24 * weekdays) ? "No" : "Yes") << NEW_LINE;
    }
    return 0;
}
