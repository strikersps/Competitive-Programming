/*
 * Problem Statement: https://www.codechef.com/problems/KFIB
 * Author: striker
 *
...
static int compute_nth_term(const int, const int);
...
static int compute_nth_term(const int n, const int k) {
    ...
    for(int i = k + 1, j = 1; i < n; ++i, ++j) {
        window_sum -= memoization[j - 1];
        window_sum += memoization[i - 1];
        memoization[i] = compute_mod(window_sum, MOD);
    }
