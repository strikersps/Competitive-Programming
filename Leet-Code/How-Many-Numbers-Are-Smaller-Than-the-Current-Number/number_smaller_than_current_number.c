...
static int * compute_smaller_number_than_current(int[], const int);
...
int main(void) {
    ...
    if(1 == scanf("%d", &test)) {
        ...
        while(test--) {
            ...
            if(1 == scanf("%d", &n)) {
                if(n < 1) {
                    CONSTRAINTS_OUT_OF_BOUND_ERROR(n, size of an arrat cannot be 0 or -ve);
                    exit(0);
                }
                int sequence[n];
                ...
                int *const freq_smaller_number = compute_smaller_number_than_current(sequence, n);
                for(register int i = 0; i < n; ++i) {
                    printf("%d ", freq_smaller_number[i]);
                }
                printf("\n");
                free(freq_smaller_number);
            } else {
                SCANF_READ_ERROR(1);
            }
        }
...
// Implementation of the solution is given below.

static int * compute_smaller_number_than_current(int sequence[], const int n) {
    int *const freq_smaller_number = calloc(n, sizeof*freq_smaller_number);
    if(freq_smaller_number) {
        int freq_map[MAX_VALUE];
        INITIALISE_INT_CONTAINER_ZERO(freq_map, MAX_VALUE * sizeof(int));
        for(register int i = 0; i < n; ++i) {
            ++freq_map[sequence[i]];
        }
        for(register int i = 0, k = 0; i < n; ++i, ++k) {
            for(register int j = 0; j < sequence[i]; ++j) {
                freq_smaller_number[k] += freq_map[j];
            }
        }
    } else {
        MEMORY_ALLOCATION_FAILED_ERROR(freq_smaller_number, n * sizeof*freq_smaller_number);
        exit(0);
    }
    return freq_smaller_number;
}
