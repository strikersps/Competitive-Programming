...
static int remove_duplicates(std :: vector <int> &, const int);
...
static int remove_duplicates(std :: vector <int> & sequence, const int n) {
    if(!n) {
        return 0;
    }
    int index = 0, i = 1;
    for(; i < n; ++i) {
        if(sequence.at(index) != sequence.at(i)) {
            sequence[++index] = sequence.at(i);
        }
    }
    return 1 + index;
}
