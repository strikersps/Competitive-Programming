...
static bool check_valid_mountain_array(const int *, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%s\n", check_valid_mountain_array(sequence, n) ? "true" : "false");
    }
    return EXIT_SUCCESS;
}

static bool check_valid_mountain_array(const int *sequence, const int n) {
    if(n < 3) {
        return false;
    }
