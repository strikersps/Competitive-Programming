...
static ll_t compute_sum(int[], const int);
static bool check_balance_sum(int[], const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%s\n", check_balance_sum(sequence, n) ? "YES" : "NO");
    }
    return EXIT_SUCCESS;
}

static ll_t compute_sum(int sequence[], const int n) {
    ll_t sequence_sum = 0;
    for(register int i = 0; i < n; ++i) {
...
static bool check_balance_sum(int sequence[], const int n) {
    ll_t right_sum = compute_sum(sequence, n);
    ll_t left_sum = 0;
    bool is_balanced = false;
    for(register int i = 0; i < n; ++i) {
        if(left_sum == (right_sum - sequence[i])) {
            is_balanced = true;
            break;
        }
        left_sum += sequence[i];
