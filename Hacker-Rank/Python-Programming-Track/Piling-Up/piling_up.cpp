...
static bool check_decreasing_order(const std :: vector <int> &, const int);
static bool check_stack_possible(const std :: vector <int> &, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        std :: cin >> n;
        assert(n > 0);
        std :: vector <int> cube_side_lengths(n);
        for(int & side_length: cube_side_lengths) {
            std :: cin >> side_length;
        }
        std :: cout << (check_stack_possible(cube_side_lengths, n) ? "Yes": "No") << NEW_LINE;
    }
    return 0;
}

static bool check_decreasing_order(const std :: vector <int> & sequence, const int n) {
...
static bool check_stack_possible(const std :: vector <int> & cube_side_lengths, const int n) {
    std :: vector <int> stacked_cubes;
    for(int start = 0, end = n - 1; start <= end; ) {
        if(cube_side_lengths.at(start) > cube_side_lengths.at(end)) {
            stacked_cubes.push_back(cube_side_lengths.at(start));
            ++start;
            continue;
        }
        stacked_cubes.push_back(cube_side_lengths.at(end));
        --end;
    }
    return check_decreasing_order(stacked_cubes, (int) stacked_cubes.size());
}
