...
static size_t count_pairs(std :: vector <int> &, const int, const int);
...
int main(void) {
    ...
    std :: cout << count_pairs(sequence, n, k) << NEW_LINE;
    return 0;
}

static size_t count_pairs(std :: vector <int> & sequence, const int n, const int k) {
    std :: sort(sequence.begin(), sequence.end());
    size_t pair_count = 0L;
    for(int i = 0; i < n; ++i) {
        if(std :: binary_search(sequence.begin() + i, sequence.end(), sequence[i] + k)) {
            ++pair_count;
        }
    }
    return pair_count;
