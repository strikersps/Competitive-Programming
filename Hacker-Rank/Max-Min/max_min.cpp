...
#define INF 0x3F3F3F3F // Infinity representation for 32-bit integers. Reference: https://stackoverflow.com/questions/18429021/why-is-infinity-0x3f3f3f3f
#define L_INF 0x3F3F3F3F3F3F3F3F // // Infinity representation when working with 64-bit representation aka long long types.
...
bool check_perfect_square(ll_t n) {
    return compute_total_divisors(n) % 2LL ? true : false;
}

/*END-OF CODE-TEMPLATE*/

static int compute_min_fairness(std :: vector <int> &, const int, const int);
...
int main(void) {
    ...
    for(int & curr_val: sequence) {
        std :: cin >> curr_val;
    }
    std :: cout << compute_min_fairness(sequence, n, k) << NEW_LINE;
    return 0;
}

static int compute_min_fairness(std :: vector <int> & sequence, const int n, const int k) {
    std :: sort(sequence.begin(), sequence.end());
    int min_val = INF;
    for(int i = 0; i <= n - k; ++i) {
        int curr_min = sequence[i + k - 1] - sequence[i];
        min_val = std :: min(min_val, curr_min);
    }
    return min_val;
}
