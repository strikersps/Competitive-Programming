...
static ll_t compute_maximum_modulo_sum(std :: vector <ll_t> &, const ll_t);
...
int main(void) {
    ...
    while(test--) {
       ...
       for (ll_t & curr_val: sequence) {
           std :: cin >> curr_val;
       }
       std :: cout << compute_maximum_modulo_sum(sequence, m) << NEW_LINE;
    }
    return 0;
}

static ll_t compute_maximum_modulo_sum(std :: vector <ll_t> & sequence, const ll_t m) {
    ll_t curr_prefix_mod = 0LL, max_modulo_sum = 0LL;
    std :: set <ll_t> prefix_mods;
    for (const ll_t & curr_val: sequence) {
        curr_prefix_mod = (curr_prefix_mod + curr_val) % m;
        max_modulo_sum = std :: max(max_modulo_sum, curr_prefix_mod);
        if(max_modulo_sum == m - 1) {
            break;
        }
        std :: set <ll_t> :: iterator loc_itr = prefix_mods.upper_bound(curr_prefix_mod);
        if(loc_itr != prefix_mods.end()) {
            max_modulo_sum = std :: max(max_modulo_sum, compute_mod((curr_prefix_mod - *loc_itr), m));
            if(max_modulo_sum == m - 1) {
                break;
            }
        }
        prefix_mods.insert(curr_prefix_mod);
    }
