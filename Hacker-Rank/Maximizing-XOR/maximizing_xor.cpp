...
static ll_t compute_maximum_xor(const ll_t, const ll_t);

int main(void) {
    FAST_IO(0);
    ll_t start_range, end_range;
    std :: cin >> start_range >> end_range;
    std :: cout << compute_maximum_xor(start_range, end_range) << NEW_LINE;
    return 0;
}

static ll_t compute_maximum_xor(const ll_t start_range, const ll_t end_range) {
    ll_t xored = start_range ^ end_range;
    return xored | ((1LL << (1LL + (ll_t) log2l(xored))) - 1LL);
}
