...
int main(void) {
    ...
    if(1 == scanf("%d", &test)) {
        ...
        while(test--) {
            int cost_green, cost_blue; // Cost of Green and Blue Balloons.
            if(2 == scanf("%d%d", &cost_green, &cost_blue)) {
                int n, count_first_solved, count_second_solved;
                count_first_solved = count_second_solved = 0;
                for(scanf("%d", &n); n > 0; --n) {
                    int first_solved, second_solved;
                    if(2 == scanf("%d%d", &first_solved, &second_solved)) {
                        ...
                        if(first_solved && second_solved) {
                            ++count_first_solved;
                            ++count_second_solved;
                        } else if(first_solved && !second_solved) {
                            ++count_first_solved;
                        } else {
                            ++count_second_solved;
                        }
                ...
                int tot_cost = 0;
                if(count_first_solved > count_second_solved) {
                    tot_cost = (cost_green < cost_blue) ? (count_first_solved * cost_green) + (count_second_solved * cost_blue) : (count_first_solved * cost_blue) + (count_second_solved * cost_green);
                } else {
                    tot_cost = (cost_green < cost_blue) ? (count_first_solved * cost_blue) + (count_second_solved * cost_green) : (count_first_solved * cost_green) + (count_second_solved * cost_blue);
                }
                printf("%d\n", tot_cost);
            } else {
                SCANF_READ_ERROR(2);
