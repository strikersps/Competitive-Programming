...
static bool check_all_zeroes(const std :: vector <int> &);
static bool check_is_possible(std :: vector <int>, const int);
...
int main(void) {
    ...
    std :: cout << (check_is_possible(sequence, n) ? "YES" : "NO") << NEW_LINE;
    return 0;
}

static bool check_all_zeroes(const std :: vector <int> & sequence) {
...
static bool check_is_possible(std :: vector <int> sequence, const int n) {
    if(check_all_zeroes(sequence)) {
        return true;
    }
    for(int i = 0; i < (n - 1); ++i) {
        if(sequence.at(i) == 0 || sequence.at(i) > sequence.at(i + 1)) {
            break;
        }
        sequence.at(i + 1) -= sequence.at(i);
        sequence.at(i) = 0;
    }
    return check_all_zeroes(sequence) ? true : false;
}
