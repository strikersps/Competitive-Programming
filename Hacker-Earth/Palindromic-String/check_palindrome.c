...
int main(void) {
    /* Hacker-Earth doesn't have ONLINE_JUDGE macro defined.
    #ifndef ONLINE_JUDGE
    ...
    */
    ...
    if(1 == scanf("%s", word)) {
        printf("%s\n", check_palindrome(word, 0, strlen(word) - 1) ? "YES" : "NO");
    } else {
        SCANF_READ_ERROR(1);
    }
    return EXIT_SUCCESS;
...
static bool check_palindrome(char word[], int start, int end) {
