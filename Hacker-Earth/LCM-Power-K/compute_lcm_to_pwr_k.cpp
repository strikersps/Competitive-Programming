/*
 * Problem Statement: https://www.hackerearth.com/problem/algorithm/lcmk-a8c3e378/?layout=old
 * Author: striker
 *
...
static ll_t compute_lcm_to_power_k(const std :: vector <int> &, const int, const int, const int, const std :: vector <int> &);
...
int main(void) {
    ...
    while(test--) {
        int n, modulus, exponent;
        std :: cin >> n >> modulus >> exponent;
        std :: vector <int> sequence(n);
        for(int & number: sequence) {
            std :: cin >> number;
        }
        std :: cout << compute_lcm_to_power_k(sequence, n, modulus, exponent, smallest_prime_factors) << NEW_LINE;
    }
    return 0;
}
...
static ll_t compute_modular_exponentiation(ll_t base, ll_t exponent, const ll_t modulus) {
    assert(modulus > 0LL);
    if(modulus == 1LL) {
        return 0LL;
    }
    ll_t result = 1LL;
    for(; exponent; exponent /= 2LL) {
        if(exponent % 2LL) {
            result = compute_mod(result * base, modulus);
        }
        base = compute_mod(base * base, modulus);
    }
    return result;
}

static ll_t compute_lcm_to_power_k(const std :: vector <int> & sequence, const int n, const int modulus, const int exponent, const std :: vector <int> & smallest_prime_factors) {
    std :: map <int, ll_t> prime_representation_sequence;
    for(int number: sequence) {
        std :: map <int, ll_t> prime_representation;
        for(; number != 1; ++prime_representation[smallest_prime_factors[number]], number /= smallest_prime_factors[number]);
        for(std :: pair <const int, ll_t> & mapping: prime_representation) {
            if(prime_representation_sequence.count(mapping.first)) {
                prime_representation_sequence[mapping.first] = std :: max(prime_representation_sequence[mapping.first], mapping.second);
                continue;
            }
            prime_representation_sequence[mapping.first] = mapping.second;
        }
    }
    ll_t lcm_value = 1LL;
    for(const std :: pair <const int, ll_t> & mapping: prime_representation_sequence) {
        lcm_value = compute_mod(lcm_value * compute_modular_exponentiation(mapping.first, exponent * mapping.second, modulus), modulus);
        if(!lcm_value) {
            break;
        }
    }
