/*
 * Problem Statement: https://cses.fi/problemset/task/1095
 * Author: striker
 *
...
int main(void) {
    ...
    while(test--) {
        int a, b;
        std :: cin >> a >> b;
        std :: cout << modular_exponentiation(a, b) << NEW_LINE;
    }
    return 0;
}
...
static ll_t modular_exponentiation(ll_t base, ll_t exponent) {
    ...
    for(; exponent; exponent >>= 1LL) {
        if(exponent % 2LL) {
            result = compute_mod(result * base, MOD);
        }
        base = compute_mod(base * base, MOD);
    }
    return result;
}
