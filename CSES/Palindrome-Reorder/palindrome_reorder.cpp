...
static std :: string string_rearrangement(std :: string & given_string, std :: vector <int> & alphabet_frequency) {
    ...
    std :: pair <char, int> odd_alphabet;
    for(int i = 0; i < 26; ++i) {
        if(alphabet_frequency.at(i) % 2) {
            odd_alphabet = {'A' + i, alphabet_frequency.at(i)};
            alphabet_frequency.at(i) = 0;
            continue;
        }
        for(int j = 0; j < alphabet_frequency.at(i) / 2; ++j) {
            palindrome_string += 'A' + i;
        }
        alphabet_frequency.at(i) /= 2;
    }
    for(int i = 0; i < odd_alphabet.second; ++i) {
        palindrome_string += odd_alphabet.first;
    }
    for(int i = 25; ~i; --i) {
        for(int j = 0; j < alphabet_frequency.at(i); ++j) {
            palindrome_string += 'A' + i;
        }
    }
