...
int main(void) {
    ...
    int n, m;
    std :: cin >> n >> m;
    std :: vector <int> biding_price(m);
    std :: multiset <int, std :: greater <int>> ticket_price;
    for(int i = 0; i < n; ++i) {
        int price;
        std :: cin >> price;
        ticket_price.emplace(price);
    }
    for(int & price: biding_price) {
        std :: cin >> price;
    }
    for(const int & price: biding_price) {
        auto found_itr = ticket_price.lower_bound(price);
        if(found_itr == ticket_price.end()) {
            std :: cout << -1 << NEW_LINE;
            continue;
        }
        std :: cout << *found_itr << NEW_LINE;
        ticket_price.erase(found_itr);
    }
    return 0;
}
