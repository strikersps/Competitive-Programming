...
static ll_t compute_number_of_bit_strings(const ll_t);
static ll_t modular_exponentiation(ll_t, ll_t, const ll_t);

int main(void) {
    FAST_IO(0);
    ll_t n;
    std :: cin >> n;
    std :: cout << compute_number_of_bit_strings(n) << std :: endl;
    return 0;
}

static ll_t compute_number_of_bit_strings(const ll_t n) {
    if(n == 1) {
        return 2;
    }
...
static ll_t modular_exponentiation(ll_t base, ll_t exponent, const ll_t modulus) {
    assert(modulus > 0);
    if(modulus == 1) {
        return 0;
    }
    ...
    for(; exponent; exponent >>= 1) {
        if(exponent & 1) {
            result = compute_mod(compute_mod(result, modulus) * compute_mod(base, modulus), modulus);
        }
        base = compute_mod(compute_mod(base, modulus) * compute_mod(base, modulus), modulus);
    }
    return result;
}
