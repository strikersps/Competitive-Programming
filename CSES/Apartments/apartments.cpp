...
static int compute_number_of_applicants(const std :: vector <int> &, std :: vector <int> &, const int);

int main(void) {
    FAST_IO(0);
    int n, m, k;
    std :: cin >> n >> m >> k;
    std :: vector <int> desired_sizes(n), apartment_sizes(m);
    for(int & size: desired_sizes) {
        std :: cin >> size;
    }
    ...
    std :: cout << compute_number_of_applicants(desired_sizes, apartment_sizes, k) << NEW_LINE;
    return 0;
}

static int compute_number_of_applicants(const std :: vector <int> & desired_sizes, std :: vector <int> & apartment_sizes, const int k) {
    int applicant_count = 0;
    for(int i = 0, j = 0; i < (int) desired_sizes.size() && j < (int) apartment_sizes.size(); ) {
        if(abs(desired_sizes[i] - apartment_sizes[j]) <= k) {
            ++applicant_count;
            ++i, ++j;
        } else if(desired_sizes[i] + k < apartment_sizes[j]) {
            ++i;
        } else {
            ++j;
        }
