...
static ll_t compute_factorial(ll_t n) {
    if(n == 1LL) {
        return 1LL;
    }
    return n * compute_factorial(n - 1LL);
}

int main(void) {
    FAST_IO(0);
    std :: string given_string, copy_given_string;
    std :: cin >> given_string;
    std :: map <char, int> alphabet_frequency;
    for(auto & character: given_string) {
        ++alphabet_frequency[character];
    }
    /*
     * NOTE: As the length of string cannot exceed 8 (refer problem description), I am using combinatorics to compute the total number of permutations using factorial function.
    */
    ll_t denominator = 1LL;
    for(const std :: pair <const char, int> & mapping: alphabet_frequency) {
        denominator *= compute_factorial(mapping.second);
    }
    std :: cout << compute_factorial(given_string.size()) / denominator << NEW_LINE;
    copy_given_string = given_string;
    std :: sort(copy_given_string.begin(), copy_given_string.end());
    std :: cout << copy_given_string << NEW_LINE;
    while(std :: next_permutation(copy_given_string.begin(), copy_given_string.end())) {
        std :: cout << copy_given_string << NEW_LINE;
    }
    return 0;
}
