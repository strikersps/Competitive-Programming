...
static int compute_minimum_gondolas(const std :: vector <int> &, const int, const int);
...
int main(void) {
    ...
    std :: cout << compute_minimum_gondolas(weights, n, x) << NEW_LINE;
    return 0;
}

static int compute_minimum_gondolas(const std :: vector <int> & weights, const int n, const int x) {
    int g_count = 0;
    for(int start = 0, end = n - 1; start <= end; --end) {
        ++g_count;
