...
static ll_t compute_largest_prime_factor(ll_t);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%lld\n", compute_largest_prime_factor(n));
    }
    return EXIT_SUCCESS;
}

static ll_t compute_largest_prime_factor(ll_t n) {
    ll_t largest_prime_factor = 1;
    if(!(n % 2)) {
        largest_prime_factor = 2;
        for(; !(n % 2); n /= 2);
    }
    for(register ll_t i = 3; i * i <= n; i += 2) {
        if(!(n % i)) {
