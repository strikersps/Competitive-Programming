...
static int compute_n_choose_r(const int, const int);
...
static int compute_n_choose_r(const int n, const int r) {
    if(n < r) {
        return 0;
    }
    if(n == r) {
    ...
    int combination[n + 1];
    INITIALISE_INT_CONTAINER_ZERO(combination, (unsigned long) (n + 1) * sizeof(int));
    combination[0] = 1;
    for(register int i = 0; i <= n; ++i) {
        for(register int j = i; j > 0; --j) {
            ...
            combination[j] = (combination[j] + combination[j - 1]) % MOD;
        }
    }
