/*
 * Problem Statement: https://practice.geeksforgeeks.org/problems/sum-of-bit-differences/0
 * Author: striker
 *
...
static int compute_sum_of_bit_difference(int[], const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%d\n", compute_sum_of_bit_difference(sequence, n) << 1);
    }
    return EXIT_SUCCESS;
}

static int compute_sum_of_bit_difference(int sequence[], const int n) {
    int sum_bit_difference = 0;
    for(register int i = 0; i < (n - 1); ++i) {
        for(register int j = i + 1; j < n; ++j) {
            sum_bit_difference += COMPUTE_SET_BITS((ull_t) sequence[i] ^ sequence[j]);
        }
    }
    return sum_bit_difference;
}
