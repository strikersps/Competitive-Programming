...
static int compute_prime_set_bit(const int, const int);
...
static int compute_prime_set_bit(const int start, const int end) {
    int count_prime_set_bit = 0;
    for(register int i = start; i <= end; ++i) {
        if(check_prime(COMPUTE_SET_BITS((ull_t) i))) {
            ++count_prime_set_bit;
            continue;
        }
    }
