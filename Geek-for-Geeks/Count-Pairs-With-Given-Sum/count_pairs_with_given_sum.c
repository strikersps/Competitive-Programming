/*
 * Problem Statement: https://practice.geeksforgeeks.org/problems/count-pairs-with-given-sum/0
 * Author: striker
 *
...
int count_pair_with_given_sum(int[], const int, const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%d\n", count_pair_with_given_sum(sequence, n, k));
    }
    return EXIT_SUCCESS;
}

/*
 * Time Complexity: O(n^2). For more efficient solution using hash map, refer to the python solution.
 * Space Complexity: O(1).
*/

int count_pair_with_given_sum(int sequence[], const int n, const int k) {
    int count_pair = 0;
    for(register int i = 0; i < (n - 1); ++i) {
        for(register int j = i + 1; j < n; ++j) {
            if(k == (sequence[i] + sequence[j])) {
                ++count_pair;
            }
        }
    }
