...
static bool check_bleak_number(const int);
...
int main(void) {
    ...
    while(test--) {
        ...
        printf("%s\n", check_bleak_number(n) ? "1" : "0");
    }
    return EXIT_SUCCESS;
}

static bool check_bleak_number(const int n) {
    bool is_bleak = true;
    for(register int i = (n - ((int) log2(n) + 1)); i < n; ++i) {
        if(n == i + COMPUTE_SET_BITS((unsigned) i)) {
            is_bleak = false;
            break;
        }
    }
