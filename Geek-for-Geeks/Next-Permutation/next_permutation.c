/*
 * Problem Statement: https://practice.geeksforgeeks.org/problems/next-permutation/0
 * Author: striker
 *
...
static void reverse_number(int[], int, int);
static void display_number(int[], const int, const char *const);
...
static void reverse_number(int number[], int start, int end) {
    for(; start < end; swap_data(&number[start], &number[end]), ++start, --end);
}
...
static bool next_permutation(int number[], const int n) {
    ...
    reverse_number(number, start, n - 1);
    return true;
}

static void display_number(int number[], int n, const char *const delimiter) {
    printf("%d", number[0]);
    for(register int i = 1; i < n; ++i) {
        printf("%s%d", delimiter, number[i]);
    }
}
