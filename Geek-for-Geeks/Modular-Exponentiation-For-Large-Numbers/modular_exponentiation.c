...
int main(void) {
    ...
    while(test--) {
        ll_t base, exponent, modular;
        if(3 != scanf("%lld%lld%lld", &base, &exponent, &modular)) {
            SCANF_READ_ERROR(3);
        }
        printf("%lld\n", modular_exponentiation(base, exponent, modular));
    }
    return EXIT_SUCCESS;
}

static ll_t modular_exponentiation(ll_t base, ll_t exponent, const ll_t modular) {
    if(modular == 1) {
        return 0;
    }
    ll_t result = 1;
