...
static bool previous_permutation(char[]);
...
static bool previous_permutation(char word[]) {
    int start, end, word_len = (int) strlen(word) - 1;
    for(start = word_len; start > 0 && word[start - 1] <= word[start]; --start);
    if(!start) {
        return false;
    }
    for(end = word_len; word[end] >= word[start - 1]; --end);
    swap_data(&word[start - 1], &word[end]);
    for(end = word_len; start < end; swap_data(&word[start], &word[end]), ++start, --end);
    return true;
}
